/**
 * collector.mjs — RSS sampler + server metrics tailer.
 *
 * Samples the server process RSS (ps -o rss=) every --interval-sec and tails the
 * server's metrics.jsonl. At exit it computes:
 ...
 *
 * Usage:
 *   node scripts/scale-validate/collector.mjs --pid 12345 \
 *     --metrics-file graphql/server/logs/metrics.jsonl --out collector.json
 */
import { spawnSync } from 'node:child_process';
import fs from 'node:fs';
import { asInt, nowIso, parseArgs, round2, sleep } from './_lib.mjs';

const HELP = `collector.mjs — sample RSS + tail metrics.jsonl, summarize at exit
...
function pickCounter(flat, family) {
  ...
  const score = (p) => (/(total|count|sum|cumulative|s$)/i.test(p) ? 2 : 0) + p.length / 1000;
  candidates.sort((a, b) => score(b.path) - score(a.path));
  return { value: candidates[0].value, path: candidates[0].path, candidates: candidates.map((c) => ({ path: c.path, value: c.value })) };
}

async function main() {
  const { args } = parseArgs(process.argv.slice(2));
  if (args.help || args.pid === undefined) {
    process.stdout.write(HELP);
    if (args.pid === undefined && !args.help) process.exitCode = 1;
    return;
  }
  const pid = asInt(args.pid, 0);
  const metricsFile = typeof args['metrics-file'] === 'string' ? args['metrics-file'] : null;
  const intervalSec = asInt(args['interval-sec'], 15);
  const durationSec = asInt(args['duration-sec'], 0);
  const out = typeof args.out === 'string' ? args.out : 'collector.json';

  const startMs = Date.now();
  const samples = []; // { tSec, rssMb }
  let lastMetricsObj = null;
  ...
  const readMetricsTail = () => {
    if (!metricsFile) return;
    let stat;
    try { stat = fs.statSync(metricsFile); } catch { return; }
  ...
  const finalize = () => {
    ...
    emit({
      t: 'summary', rssMinMb: rssMin, rssMaxMb: rssMax, rssFinalMb: rssFinal, slopeMbPerHour,
      evictionsTotal: evictions.value, drainsTotal: drains.value, buildsTotal: builds.value, out
    });
    return summary;
  };

  const onSig = () => { stopping = true; stopReason = stopReason || 'signal'; };
  process.on('SIGINT', onSig);
  process.on('SIGTERM', onSig);

  // Prime the metrics offset so we start reading from current EOF-ish but still
  // capture the last line: read everything once so lastMetricsObj is populated.
  readMetricsTail();

  while (!stopping) {
    const rssMb = sampleRssMb(pid);
    if (rssMb == null) { stopReason = 'pid-exited'; break; }
    ...
    if (durationSec > 0 && (Date.now() - startMs) / 1000 >= durationSec) { stopReason = 'duration'; break; }
