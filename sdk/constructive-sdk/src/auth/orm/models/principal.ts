...
import type { Principal, PrincipalWithRelations, PrincipalSelect, PrincipalFilter, PrincipalOrderBy, CreatePrincipalInput, UpdatePrincipalInput, PrincipalPatch } from "../input-types";
...
export class PrincipalModel {
  ...
  findMany<S extends PrincipalSelect>(args: FindManyArgs<S, PrincipalFilter, PrincipalOrderBy> & {
    ...
    const {
      document,
      variables
    } = buildFindManyDocument("Principal", "principals", args.select, {
      where: args?.where,
  ...
  findFirst<S extends PrincipalSelect>(args: FindFirstArgs<S, PrincipalFilter, PrincipalOrderBy> & {
    ...
    const {
      document,
      variables
    } = buildFindFirstDocument("Principal", "principals", args.select, {
      where: args?.where,
      orderBy: args?.orderBy as string[] | undefined
    }, "PrincipalFilter", "PrincipalOrderBy", connectionFieldsMap);
    return new QueryBuilder({
  ...
  create<S extends PrincipalSelect>(args: CreateArgs<S, CreatePrincipalInput["principal"]> & {
    ...
    const {
      document,
      variables
    } = buildCreateDocument("Principal", "createPrincipal", "principal", args.select, args.data, "CreatePrincipalInput", connectionFieldsMap);
    return new QueryBuilder({
      client: this.client,
      operation: "mutation",
      operationName: "Principal",
