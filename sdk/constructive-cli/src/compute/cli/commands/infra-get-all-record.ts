/**
 * CLI commands for InfraGetAllRecord
  ...
  try {
    ...
    const findManyArgs = parseFindManyArgs<
      FindManyArgs<InfraGetAllRecordSelect, InfraGetAllRecordFilter, InfraGetAllRecordsOrderBy> & {
        select: InfraGetAllRecordSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.infraGetAllRecord.findMany(findManyArgs).execute();
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateInfraGetAllRecordInput['infraGetAllRecord'];
    const client = getClient();
