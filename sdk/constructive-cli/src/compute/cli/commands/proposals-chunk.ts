/**
 * CLI commands for ProposalsChunk
...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findManyArgs = parseFindManyArgs<
      FindManyArgs<ProposalsChunkSelect, ProposalsChunkFilter, ProposalsChunkOrderBy> & {
    ...
    const result = await client.proposalsChunk.findMany(findManyArgs).execute();
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findFirstArgs = parseFindFirstArgs<
      FindFirstArgs<ProposalsChunkSelect, ProposalsChunkFilter, ProposalsChunkOrderBy> & {
        select: ProposalsChunkSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
...
async function handleGet(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const client = getClient();
    const result = await client.proposalsChunk
      .findOne({
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateProposalsChunkInput['proposalsChunk'];
    if (argv['auto-embed']) {
      const embedder = resolveEmbedder();
      if (!embedder) {
        console.error(
    ...
    const result = await client.proposalsChunk
      .create({
        data: {
          ...
          proposalsId: cleanedData.proposalsId,
        },
        select: {
          actorId: true,
          body: true,
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as ProposalsChunkPatch;
    if (argv['auto-embed']) {
      const embedder = resolveEmbedder();
      if (!embedder) {
        console.error(
