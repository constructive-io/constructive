/**
 * CLI commands for RegistryBinding
...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    const defaultSelect = {
      ...
      metadata: true,
      namespaceId: true,
      observedCredentialVersion: true,
      pullSecretName: true,
      realm: true,
      registryHost: true,
      registryId: true,
    ...
    const findManyArgs = parseFindManyArgs<
      FindManyArgs<RegistryBindingSelect, RegistryBindingFilter, RegistryBindingOrderBy> & {
        select: RegistryBindingSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.registryBinding.findMany(findManyArgs).execute();
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findFirstArgs = parseFindFirstArgs<
      FindFirstArgs<RegistryBindingSelect, RegistryBindingFilter, RegistryBindingOrderBy> & {
        select: RegistryBindingSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
...
async function handleGet(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const client = getClient();
    const result = await client.registryBinding
      .findOne({
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    const rawAnswers = await prompter.prompt(argv, [
      ...
      {
        type: 'text',
        name: 'observedCredentialVersion',
        message: 'observedCredentialVersion',
        required: false,
        skipPrompt: true,
      },
      {
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateRegistryBindingInput['registryBinding'];
    const client = getClient();
    const result = await client.registryBinding
      .create({
        data: {
          ...
          observedCredentialVersion: cleanedData.observedCredentialVersion,
          pullSecretName: cleanedData.pullSecretName,
          realm: cleanedData.realm,
          registryHost: cleanedData.registryHost,
          registryId: cleanedData.registryId,
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as RegistryBindingPatch;
    const client = getClient();
    const result = await client.registryBinding
      .update({
        where: {
