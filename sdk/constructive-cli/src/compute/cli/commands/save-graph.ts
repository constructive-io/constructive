/**
 * CLI command for mutation saveGraph
...
import type { SaveGraphPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.saveGraph(parsedAnswers as unknown as SaveGraphVariables, {
      select: selectFields
    } as unknown as {
      select: SaveGraphPayloadSelect;
    }).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    console.error("Failed: saveGraph");
