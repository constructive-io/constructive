/**
 * CLI command for mutation addNodeAndSave
...
import type { AddNodeAndSavePayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.addNodeAndSave(parsedAnswers as unknown as AddNodeAndSaveVariables, {
      select: selectFields
    } as unknown as {
      select: AddNodeAndSavePayloadSelect;
    }).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    console.error("Failed: addNodeAndSave");
