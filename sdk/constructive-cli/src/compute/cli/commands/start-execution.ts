/**
 * CLI command for mutation startExecution
...
import type { StartExecutionPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.startExecution(parsedAnswers as unknown as StartExecutionVariables, {
      select: selectFields
    } as unknown as {
      select: StartExecutionPayloadSelect;
    }).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    console.error("Failed: startExecution");
