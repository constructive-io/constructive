/**
 * CLI command for mutation platformResourceInstallationsInstall
...
import type { PlatformResourceInstallationsInstallPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.platformResourceInstallationsInstall(parsedAnswers as unknown as PlatformResourceInstallationsInstallVariables, {
      select: selectFields
    } as unknown as {
      select: PlatformResourceInstallationsInstallPayloadSelect;
    }).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    console.error("Failed: platformResourceInstallationsInstall");
