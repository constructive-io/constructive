/**
 * CLI command for mutation copyGraph
...
import type { CopyGraphPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.copyGraph(parsedAnswers as unknown as CopyGraphVariables, {
      select: selectFields
    } as unknown as {
      select: CopyGraphPayloadSelect;
    }).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    console.error("Failed: copyGraph");
