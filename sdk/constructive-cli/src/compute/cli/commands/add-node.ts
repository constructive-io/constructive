/**
 * CLI command for mutation addNode
...
import type { AddNodePayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.addNode(parsedAnswers as unknown as AddNodeVariables, {
      select: selectFields
    } as unknown as {
      select: AddNodePayloadSelect;
    }).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    console.error("Failed: addNode");
