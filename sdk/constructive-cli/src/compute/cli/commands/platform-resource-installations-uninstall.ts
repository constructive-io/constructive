/**
 * CLI command for mutation platformResourceInstallationsUninstall
...
import type { PlatformResourceInstallationsUninstallPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.platformResourceInstallationsUninstall(parsedAnswers as unknown as PlatformResourceInstallationsUninstallVariables, {
      select: selectFields
    } as unknown as {
      select: PlatformResourceInstallationsUninstallPayloadSelect;
    }).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    console.error("Failed: platformResourceInstallationsUninstall");
