/**
 * CLI command for mutation resourceInstallationsRollback
...
import type { ResourceInstallationsRollbackPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.resourceInstallationsRollback(parsedAnswers as unknown as ResourceInstallationsRollbackVariables, {
      select: selectFields
    } as unknown as {
      select: ResourceInstallationsRollbackPayloadSelect;
    }).execute();
