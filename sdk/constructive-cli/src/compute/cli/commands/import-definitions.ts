/**
 * CLI command for mutation importDefinitions
...
import type { ImportDefinitionsPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.importDefinitions(parsedAnswers as unknown as ImportDefinitionsVariables, {
      select: selectFields
    } as unknown as {
      select: ImportDefinitionsPayloadSelect;
    }).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    console.error("Failed: importDefinitions");
