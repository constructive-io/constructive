/**
 * CLI command for mutation importGraphJson
...
import type { ImportGraphJsonPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.importGraphJson(parsedAnswers as unknown as ImportGraphJsonVariables, {
      select: selectFields
    } as unknown as {
      select: ImportGraphJsonPayloadSelect;
    }).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    console.error("Failed: importGraphJson");
