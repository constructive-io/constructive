/**
 * CLI commands for PlatformProposalsChunk
...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.platformProposalsChunk.findMany(findManyArgs).execute();
...
async function handleGet(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const client = getClient();
    const result = await client.platformProposalsChunk
      .findOne({
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    const rawAnswers = await prompter.prompt(argv, [
      ...
      {
        type: 'text',
        name: 'platformProposalsId',
        message: 'platformProposalsId',
        required: true,
      },
    ]);
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreatePlatformProposalsChunkInput['platformProposalsChunk'];
    if (argv['auto-embed']) {
      const embedder = resolveEmbedder();
      if (!embedder) {
        console.error(
    ...
    const result = await client.platformProposalsChunk
      .create({
        data: {
          ...
          platformProposalsId: cleanedData.platformProposalsId,
        },
        select: {
          actorId: true,
          body: true,
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as PlatformProposalsChunkPatch;
    if (argv['auto-embed']) {
      const embedder = resolveEmbedder();
      if (!embedder) {
        console.error(
