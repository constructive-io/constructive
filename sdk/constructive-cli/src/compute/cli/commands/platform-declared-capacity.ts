/**
 * CLI commands for PlatformDeclaredCapacity
  ...
  try {
    ...
    const findManyArgs = parseFindManyArgs<
      FindManyArgs<
        ...
        PlatformDeclaredCapacityOrderBy
      > & {
        select: PlatformDeclaredCapacitySelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.platformDeclaredCapacity.findMany(findManyArgs).execute();
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreatePlatformDeclaredCapacityInput['platformDeclaredCapacity'];
    const client = getClient();
    const result = await client.platformDeclaredCapacity
      .create({
        data: {
