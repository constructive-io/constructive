/**
 * CLI command for mutation resourceInstallationsUninstall
...
import type { ResourceInstallationsUninstallPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.resourceInstallationsUninstall(parsedAnswers as unknown as ResourceInstallationsUninstallVariables, {
      select: selectFields
    } as unknown as {
      select: ResourceInstallationsUninstallPayloadSelect;
    }).execute();
