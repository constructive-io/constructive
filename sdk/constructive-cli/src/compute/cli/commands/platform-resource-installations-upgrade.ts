/**
 * CLI command for mutation platformResourceInstallationsUpgrade
...
import type { PlatformResourceInstallationsUpgradePayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.platformResourceInstallationsUpgrade(parsedAnswers as unknown as PlatformResourceInstallationsUpgradeVariables, {
      select: selectFields
    } as unknown as {
      select: PlatformResourceInstallationsUpgradePayloadSelect;
    }).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    console.error("Failed: platformResourceInstallationsUpgrade");
