...
import type { SendVerificationEmailPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.sendVerificationEmail(parsedAnswers as unknown as SendVerificationEmailVariables, {
      select: selectFields
    } as unknown as {
      select: SendVerificationEmailPayloadSelect;
    }).execute();
