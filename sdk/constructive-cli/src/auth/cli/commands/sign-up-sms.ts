...
import type { SignUpSmsPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.signUpSms(parsedAnswers as unknown as SignUpSmsVariables, {
      select: selectFields
    } as unknown as {
      select: SignUpSmsPayloadSelect;
    }).execute();
