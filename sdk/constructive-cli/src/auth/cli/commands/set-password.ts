/**
 * CLI command for mutation setPassword
...
import type { SetPasswordPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.setPassword(parsedAnswers as unknown as SetPasswordVariables, {
      select: selectFields
    } as unknown as {
      select: SetPasswordPayloadSelect;
    }).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    console.error("Failed: setPassword");
