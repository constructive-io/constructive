/**
 * CLI commands for UserSettingsSecurity
...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.userSettingsSecurity.findMany(findManyArgs).execute();
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.userSettingsSecurity.findFirst(findFirstArgs).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateUserSettingsSecurityInput['userSettingsSecurity'];
    const client = getClient();
    const result = await client.userSettingsSecurity
      .create({
        data: {
          backupCodesCount: cleanedData.backupCodesCount,
          emailMfaEnabled: cleanedData.emailMfaEnabled,
          mfaEnrolledAt: cleanedData.mfaEnrolledAt,
          mfaLastUsedAt: cleanedData.mfaLastUsedAt,
          ownerId: cleanedData.ownerId,
          smsMfaEnabled: cleanedData.smsMfaEnabled,
          totpEnabled: cleanedData.totpEnabled,
        },
        select: {
          backupCodesCount: true,
          createdAt: true,
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as UserSettingsSecurityPatch;
    const client = getClient();
    const result = await client.userSettingsSecurity
      .update({
        where: {
