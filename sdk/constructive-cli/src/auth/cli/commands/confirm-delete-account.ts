/**
 * CLI command for mutation confirmDeleteAccount
...
import type { ConfirmDeleteAccountPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.confirmDeleteAccount(parsedAnswers as unknown as ConfirmDeleteAccountVariables, {
      select: selectFields
    } as unknown as {
      select: ConfirmDeleteAccountPayloadSelect;
    }).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    console.error("Failed: confirmDeleteAccount");
