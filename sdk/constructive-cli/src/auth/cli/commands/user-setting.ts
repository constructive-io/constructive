...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findManyArgs = parseFindManyArgs<
      FindManyArgs<UserSettingSelect, UserSettingFilter, UserSettingOrderBy> & {
        select: UserSettingSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.userSetting.findMany(findManyArgs).execute();
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findFirstArgs = parseFindFirstArgs<
      FindFirstArgs<UserSettingSelect, UserSettingFilter, UserSettingOrderBy> & {
        select: UserSettingSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.userSetting.findFirst(findFirstArgs).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
