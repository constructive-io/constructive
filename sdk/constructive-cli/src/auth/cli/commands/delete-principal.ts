...
import type { DeletePrincipalPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.deletePrincipal(parsedAnswers as unknown as DeletePrincipalVariables, {
      select: selectFields
    } as unknown as {
      select: DeletePrincipalPayloadSelect;
    }).execute();
