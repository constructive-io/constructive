/**
 * CLI command for mutation checkPassword
...
import type { CheckPasswordPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.checkPassword(parsedAnswers as unknown as CheckPasswordVariables, {
      select: selectFields
    } as unknown as {
      select: CheckPasswordPayloadSelect;
    }).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    console.error("Failed: checkPassword");
