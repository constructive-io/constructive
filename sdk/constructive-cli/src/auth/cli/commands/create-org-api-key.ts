...
import type { CreateOrgApiKeyPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.createOrgApiKey(parsedAnswers as unknown as CreateOrgApiKeyVariables, {
      select: selectFields
    } as unknown as {
      select: CreateOrgApiKeyPayloadSelect;
    }).execute();
