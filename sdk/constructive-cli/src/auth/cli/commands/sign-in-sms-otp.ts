...
import type { SignInSmsOtpPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.signInSmsOtp(parsedAnswers as unknown as SignInSmsOtpVariables, {
      select: selectFields
    } as unknown as {
      select: SignInSmsOtpPayloadSelect;
    }).execute();
