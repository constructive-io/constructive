/**
 * CLI command for mutation signOut
...
import type { SignOutPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.signOut(parsedAnswers as unknown as SignOutVariables, {
      select: selectFields
    } as unknown as {
      select: SignOutPayloadSelect;
    }).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    console.error("Failed: signOut");
