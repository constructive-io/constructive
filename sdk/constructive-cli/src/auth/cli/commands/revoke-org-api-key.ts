...
import type { RevokeOrgApiKeyPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.revokeOrgApiKey(parsedAnswers as unknown as RevokeOrgApiKeyVariables, {
      select: selectFields
    } as unknown as {
      select: RevokeOrgApiKeyPayloadSelect;
    }).execute();
