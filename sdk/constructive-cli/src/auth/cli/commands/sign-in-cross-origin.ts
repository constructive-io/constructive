...
import type { SignInCrossOriginPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.signInCrossOrigin(parsedAnswers as unknown as SignInCrossOriginVariables, {
      select: selectFields
    } as unknown as {
      select: SignInCrossOriginPayloadSelect;
    }).execute();
