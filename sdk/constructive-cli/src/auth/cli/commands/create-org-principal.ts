...
import type { CreateOrgPrincipalPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.createOrgPrincipal(parsedAnswers as unknown as CreateOrgPrincipalVariables, {
      select: selectFields
    } as unknown as {
      select: CreateOrgPrincipalPayloadSelect;
    }).execute();
