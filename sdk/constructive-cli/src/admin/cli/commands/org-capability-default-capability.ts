/**
 * CLI commands for OrgCapabilityDefaultCapability
...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const result = await client.orgCapabilityDefaultCapability.findMany(findManyArgs).execute();
...
async function handleGet(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const client = getClient();
    const result = await client.orgCapabilityDefaultCapability
      .findOne({
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateOrgCapabilityDefaultCapabilityInput['orgCapabilityDefaultCapability'];
    const client = getClient();
    const result = await client.orgCapabilityDefaultCapability
      .create({
        data: {
          capabilityId: cleanedData.capabilityId,
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as OrgCapabilityDefaultCapabilityPatch;
    const client = getClient();
    const result = await client.orgCapabilityDefaultCapability
      .update({
        where: {
