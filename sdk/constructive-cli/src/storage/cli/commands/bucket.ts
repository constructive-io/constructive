...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findManyArgs = parseFindManyArgs<
      FindManyArgs<BucketSelect, BucketFilter, BucketOrderBy> & {
        select: BucketSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.bucket.findMany(findManyArgs).execute();
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findFirstArgs = parseFindFirstArgs<
      FindFirstArgs<BucketSelect, BucketFilter, BucketOrderBy> & {
        select: BucketSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
...
async function handleGet(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const client = getClient();
    const result = await client.bucket
      .findOne({
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(answers, fieldSchema) as CreateBucketInput['bucket'];
    const client = getClient();
    const result = await client.bucket
      .create({
        data: {
          ...
          destinationBucketId: cleanedData.destinationBucketId,
          isPublic: cleanedData.isPublic,
          key: cleanedData.key,
          maxFileSize: cleanedData.maxFileSize,
          physicalName: cleanedData.physicalName,
          stagingTtl: cleanedData.stagingTtl,
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as BucketPatch;
    const client = getClient();
    const result = await client.bucket
      .update({
        where: {
