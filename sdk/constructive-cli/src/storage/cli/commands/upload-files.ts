/**
 * CLI command for mutation uploadFiles
...
import type { UploadFileBulkPayloadSelect } from '../../orm/input-types';
export default async (
  ...
  try {
    ...
    const result = await client.mutation
      .uploadFiles(
        parsedAnswers as unknown as UploadFilesVariables,
        {
          select: selectFields,
        } as unknown as {
          select: UploadFileBulkPayloadSelect;
        }
      )
      .execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    console.error('Failed: uploadFiles');
