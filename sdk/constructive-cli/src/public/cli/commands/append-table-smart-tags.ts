/**
 * CLI command for mutation appendTableSmartTags
...
import type { AppendTableSmartTagsPayloadSelect } from '../../orm/input-types';
  ...
  try {
    ...
    const result = await client.mutation
      .appendTableSmartTags(
        parsedAnswers as unknown as AppendTableSmartTagsVariables,
        {
          select: selectFields,
        } as unknown as {
          select: AppendTableSmartTagsPayloadSelect;
        }
      )
      .execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    console.error('Failed: appendTableSmartTags');
