/**
 * CLI commands for InternalConfigModule
...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    const defaultSelect = {
      ...
      entityTableId: true,
      id: true,
      internalConfigTableId: true,
      internalConfigTableName: true,
      policies: true,
      prefix: true,
      privateApiName: true,
    ...
    const result = await client.internalConfigModule.findMany(findManyArgs).execute();
...
async function handleGet(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const client = getClient();
    const result = await client.internalConfigModule
      .findOne({
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    const rawAnswers = await prompter.prompt(argv, [
      ...
      {
        type: 'text',
        name: 'internalConfigTableId',
        message: 'internalConfigTableId',
        required: false,
        skipPrompt: true,
      },
      {
        type: 'text',
        name: 'internalConfigTableName',
        message: 'internalConfigTableName',
        required: false,
        skipPrompt: true,
      },
      {
    ...
    const cleanedData = stripUndefined(
      answers,
      fieldSchema
    ) as CreateInternalConfigModuleInput['internalConfigModule'];
    const client = getClient();
    const result = await client.internalConfigModule
      .create({
        data: {
          ...
          internalConfigTableId: cleanedData.internalConfigTableId,
          internalConfigTableName: cleanedData.internalConfigTableName,
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as InternalConfigModulePatch;
    const client = getClient();
    const result = await client.internalConfigModule
      .update({
        where: {
