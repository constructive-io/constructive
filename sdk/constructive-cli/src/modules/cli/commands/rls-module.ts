...
import type { CreateRlsModuleInput, RlsModulePatch, RlsModuleSelect, RlsModuleFilter, RlsModuleOrderBy } from "../../orm/input-types";
...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findManyArgs = parseFindManyArgs<FindManyArgs<RlsModuleSelect, RlsModuleFilter, RlsModuleOrderBy> & {
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findFirstArgs = parseFindFirstArgs<FindFirstArgs<RlsModuleSelect, RlsModuleFilter, RlsModuleOrderBy> & {
      select: RlsModuleSelect;
    }>(argv, defaultSelect);
    const client = getClient();
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(answers, fieldSchema) as CreateRlsModuleInput["rlsModule"];
    const client = getClient();
    const result = await client.rlsModule.create({
      data: {
        apiName: cleanedData.apiName,
        authenticate: cleanedData.authenticate,
        authenticateStrict: cleanedData.authenticateStrict,
        currentRole: cleanedData.currentRole,
        currentRoleId: cleanedData.currentRoleId,
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as RlsModulePatch;
    const client = getClient();
    const result = await client.rlsModule.update({
      where: {
        id: answers.id as string
      },
      data: {
...
async function handleDelete(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const client = getClient();
    const result = await client.rlsModule.delete({
      where: {
        id: answers.id as string
      },
      select: {
