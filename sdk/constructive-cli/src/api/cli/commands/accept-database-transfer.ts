/**
 * CLI command for mutation acceptDatabaseTransfer
...
import type { AcceptDatabaseTransferPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.acceptDatabaseTransfer(parsedAnswers as unknown as AcceptDatabaseTransferVariables, {
      select: selectFields
    } as unknown as {
      select: AcceptDatabaseTransferPayloadSelect;
    }).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    console.error("Failed: acceptDatabaseTransfer");
