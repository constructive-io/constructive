...
import type { RejectDatabaseTransferPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.rejectDatabaseTransfer(parsedAnswers as unknown as RejectDatabaseTransferVariables, {
      select: selectFields
    } as unknown as {
      select: RejectDatabaseTransferPayloadSelect;
    }).execute();
