/**
 * CLI command for mutation applyRls
...
import type { ApplyRlsPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.applyRls(parsedAnswers as unknown as ApplyRlsVariables, {
      select: selectFields
    } as unknown as {
      select: ApplyRlsPayloadSelect;
    }).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    console.error("Failed: applyRls");
