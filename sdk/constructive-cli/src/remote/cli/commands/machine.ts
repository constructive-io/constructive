...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findManyArgs = parseFindManyArgs<
      FindManyArgs<MachineSelect, MachineFilter, MachineOrderBy> & {
        select: MachineSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
    const result = await client.machine.findMany(findManyArgs).execute();
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findFirstArgs = parseFindFirstArgs<
      FindFirstArgs<MachineSelect, MachineFilter, MachineOrderBy> & {
        select: MachineSelect;
      }
    >(argv, defaultSelect);
    const client = getClient();
...
async function handleGet(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const client = getClient();
    const result = await client.machine
      .findOne({
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(answers, fieldSchema) as CreateMachineInput['machine'];
    const client = getClient();
    const result = await client.machine
      .create({
        data: {
          ...
          facts: cleanedData.facts,
          isShared: cleanedData.isShared,
          label: cleanedData.label,
          lastSeenAt: cleanedData.lastSeenAt,
          ownerId: cleanedData.ownerId,
          policy: cleanedData.policy,
          principalId: cleanedData.principalId,
          revokedAt: cleanedData.revokedAt,
          tokenHash: cleanedData.tokenHash,
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as MachinePatch;
    const client = getClient();
    const result = await client.machine
      .update({
        where: {
