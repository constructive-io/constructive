/**
 * CLI command for mutation platformSecretsRotate
...
import type { PlatformSecretsRotatePayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.platformSecretsRotate(parsedAnswers as unknown as PlatformSecretsRotateVariables, {
      select: selectFields
    } as unknown as {
      select: PlatformSecretsRotatePayloadSelect;
    }).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    console.error("Failed: platformSecretsRotate");
