...
import type { _SecretsDelPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation._secretsDel(parsedAnswers as unknown as _secretsDelVariables, {
      select: selectFields
    } as unknown as {
      select: _SecretsDelPayloadSelect;
    }).execute();
