/**
 * CLI command for mutation seedOrgLimitCapsDefaults
...
import type { SeedOrgLimitCapsDefaultsPayloadSelect } from "../../orm/input-types";
export default async (argv: Partial<Record<string, unknown>>, prompter: Inquirerer, _options: CLIOptions) => {
  try {
    ...
    const result = await client.mutation.seedOrgLimitCapsDefaults(parsedAnswers as unknown as SeedOrgLimitCapsDefaultsVariables, {
      select: selectFields
    } as unknown as {
      select: SeedOrgLimitCapsDefaultsPayloadSelect;
    }).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
    console.error("Failed: seedOrgLimitCapsDefaults");
