...
import type { CreateAppLimitCapInput, AppLimitCapPatch, AppLimitCapSelect, AppLimitCapFilter, AppLimitCapOrderBy } from "../../orm/input-types";
...
async function handleList(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findManyArgs = parseFindManyArgs<FindManyArgs<AppLimitCapSelect, AppLimitCapFilter, AppLimitCapOrderBy> & {
      select: AppLimitCapSelect;
    }>(argv, defaultSelect);
    const client = getClient();
    const result = await client.appLimitCap.findMany(findManyArgs).execute();
...
async function handleFindFirst(argv: Partial<Record<string, unknown>>, _prompter: Inquirerer) {
  try {
    ...
    const findFirstArgs = parseFindFirstArgs<FindFirstArgs<AppLimitCapSelect, AppLimitCapFilter, AppLimitCapOrderBy> & {
      select: AppLimitCapSelect;
    }>(argv, defaultSelect);
    const client = getClient();
    const result = await client.appLimitCap.findFirst(findFirstArgs).execute();
    console.log(JSON.stringify(result, null, 2));
  } catch (error) {
...
async function handleGet(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const client = getClient();
    const result = await client.appLimitCap.findOne({
...
async function handleCreate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const cleanedData = stripUndefined(answers, fieldSchema) as CreateAppLimitCapInput["appLimitCap"];
    const client = getClient();
    const result = await client.appLimitCap.create({
      data: {
...
async function handleUpdate(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const cleanedData = stripUndefined(answers, fieldSchema) as AppLimitCapPatch;
    const client = getClient();
    const result = await client.appLimitCap.update({
      where: {
        id: answers.id as string
      },
      data: {
...
async function handleDelete(argv: Partial<Record<string, unknown>>, prompter: Inquirerer) {
  try {
    ...
    const answers = coerceAnswers(rawAnswers, fieldSchema);
    const client = getClient();
    const result = await client.appLimitCap.delete({
      where: {
        id: answers.id as string
      },
      select: {
