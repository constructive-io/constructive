 ...
 * ```
 */
export function useCreateIdentityProviderMutation<S extends IdentityProviderSelect>(params: {
  selection: ({
...
export function useCreateIdentityProviderMutation(params: {
...
} & Omit<UseMutationOptions<any, Error, CreateIdentityProviderInput["identityProvider"]>, "mutationFn">) {
  ...
  return useMutation({
    ...
    mutationFn: (data: CreateIdentityProviderInput["identityProvider"]) => getClient().identityProvider.create({
      data,
      select: args.select
    }).unwrap(),
    onSuccess: () => {
