...
export function useLinkIdentityMutation<S extends LinkIdentityPayloadSelect>(params: {
  selection: ({
  ...
  return useMutation({
    ...
    mutationFn: (variables: LinkIdentityVariables) => getClient().mutation.linkIdentity(variables, {
      select: args.select
    } as {
      select: S;
    } & StrictSelect<S, LinkIdentityPayloadSelect>).unwrap(),
    ...mutationOptions
  });
}
