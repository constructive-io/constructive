...
export function useSignInSmsOtpMutation<S extends SignInSmsOtpPayloadSelect>(params: {
  selection: ({
  ...
  return useMutation({
    ...
    mutationFn: (variables: SignInSmsOtpVariables) => getClient().mutation.signInSmsOtp(variables, {
      select: args.select
    } as {
      select: S;
    } & StrictSelect<S, SignInSmsOtpPayloadSelect>).unwrap(),
    ...mutationOptions
  });
}
