 ...
 * ```
 */
export function useApiQuery<S extends ApiSelect, TData = {
...
export function useApiQuery(params: {
  id: string;
 ...
 * ```
 */
export async function prefetchApiQuery<S extends ApiSelect>(queryClient: QueryClient, params: {
  id: string;
  selection: {
    fields: S;
  } & HookStrictSelect<NoInfer<S>, ApiSelect>;
}): Promise<void>;
export async function prefetchApiQuery(queryClient: QueryClient, params: {
  id: string;
  selection: SelectionConfig<ApiSelect>;
}): Promise<void> {
  const args = buildSelectionArgs<ApiSelect>(params.selection);
