-- =============================================================================
-- Constructive Upload System -- object_store_public schema
-- =============================================================================
-- Run: psql -h localhost -U postgres -d constructive < migrations/object_store.sql
-- =============================================================================
...
CREATE SCHEMA IF NOT EXISTS object_store_public;

-- ---------------------------------------------------------------------------
-- 1. Status ENUM
-- ---------------------------------------------------------------------------

CREATE TYPE object_store_public.file_status AS ENUM (
  'pending',
  'processing',
  'ready',
  'error',
...
COMMENT ON TYPE object_store_public.file_status IS
...
CREATE TABLE object_store_public.files (
  id            uuid          NOT NULL DEFAULT gen_random_uuid(),
  database_id   integer       NOT NULL,
  bucket_key    text          NOT NULL DEFAULT 'default',
  key           text          NOT NULL,
  status        object_store_public.file_status NOT NULL DEFAULT 'pending',
  status_reason text,
  etag          text,
  source_table  text,
  source_column text,
...
COMMENT ON TABLE object_store_public.files IS
...
COMMENT ON COLUMN object_store_public.files.source_column IS
  'Column name on the source table (e.g. profile_picture). NULL until domain trigger populates it.';
COMMENT ON COLUMN object_store_public.files.source_id IS
...
CREATE TABLE object_store_public.buckets (
  id            uuid          NOT NULL DEFAULT gen_random_uuid(),
  database_id   integer       NOT NULL,
  key           text          NOT NULL,
  name          text          NOT NULL,
...
COMMENT ON TABLE object_store_public.buckets IS
...
CREATE OR REPLACE FUNCTION object_store_public.files_after_insert_queue_processing()
RETURNS trigger AS $$
BEGIN
...
CREATE TRIGGER files_after_insert_queue_processing
  AFTER INSERT ON object_store_public.files
  FOR EACH ROW
  WHEN (NEW.status = 'pending')
  EXECUTE FUNCTION object_store_public.files_after_insert_queue_processing();

COMMENT ON TRIGGER files_after_insert_queue_processing ON object_store_public.files IS
...
CREATE OR REPLACE FUNCTION object_store_public.files_before_update_timestamp()
RETURNS trigger AS $$
BEGIN
...
CREATE TRIGGER files_before_update_timestamp
  BEFORE UPDATE ON object_store_public.files
  FOR EACH ROW
  EXECUTE FUNCTION object_store_public.files_before_update_timestamp();

COMMENT ON TRIGGER files_before_update_timestamp ON object_store_public.files IS
...
CREATE OR REPLACE FUNCTION object_store_public.files_after_update_queue_deletion()
RETURNS trigger AS $$
BEGIN
  PERFORM app_jobs.add_job(
    'delete_s3_object',
...
CREATE TRIGGER files_after_update_queue_deletion
  AFTER UPDATE ON object_store_public.files
  FOR EACH ROW
  WHEN (NEW.status = 'deleting' AND OLD.status <> 'deleting')
  EXECUTE FUNCTION object_store_public.files_after_update_queue_deletion();

COMMENT ON TRIGGER files_after_update_queue_deletion ON object_store_public.files IS
...
CREATE OR REPLACE FUNCTION object_store_public.files_after_update_queue_retry()
RETURNS trigger AS $$
BEGIN
  PERFORM app_jobs.add_job(
    'process-image',
...
CREATE TRIGGER files_after_update_queue_retry
  ...
  EXECUTE FUNCTION object_store_public.files_after_update_queue_retry();

COMMENT ON TRIGGER files_after_update_queue_retry ON object_store_public.files IS
...
CREATE POLICY files_public_bucket_read ON object_store_public.files
  ...
  USING (
    EXISTS (
      SELECT 1 FROM object_store_public.buckets b
...
GRANT SELECT, INSERT, UPDATE, DELETE ON object_store_public.files TO service_role;

COMMENT ON POLICY files_tenant_isolation ON object_store_public.files IS
  'Every query is scoped to the current tenant via app.database_id session variable.';
COMMENT ON POLICY files_visibility ON object_store_public.files IS
  'Users see all ready files in their tenant. Non-ready files visible only to the uploader.';
COMMENT ON POLICY files_public_bucket_read ON object_store_public.files IS
  'Allows unauthenticated reads on ready files in public buckets.';
COMMENT ON POLICY files_admin_override ON object_store_public.files IS
...
CREATE OR REPLACE FUNCTION object_store_public.populate_file_back_reference()
RETURNS trigger AS $$
DECLARE
...
COMMENT ON FUNCTION object_store_public.populate_file_back_reference() IS
...
CREATE OR REPLACE FUNCTION object_store_public.mark_files_deleting_on_source_delete()
RETURNS trigger AS $$
DECLARE
  col_name text := TG_ARGV[0];
  table_name text := TG_ARGV[1];
...
COMMENT ON FUNCTION object_store_public.mark_files_deleting_on_source_delete() IS
...
BEGIN
  ...
  IF FOUND THEN
    EXECUTE 'CREATE TRIGGER users_profile_picture_file_ref
      ...
      FOR EACH ROW
      EXECUTE FUNCTION object_store_public.populate_file_back_reference(''profile_picture'', ''constructive_users_public.users'')';
    EXECUTE 'CREATE TRIGGER users_profile_picture_file_delete
      BEFORE DELETE ON constructive_users_public.users
      FOR EACH ROW
      EXECUTE FUNCTION object_store_public.mark_files_deleting_on_source_delete(''profile_picture'', ''constructive_users_public.users'')';
  ...
  IF FOUND THEN
    EXECUTE 'CREATE TRIGGER app_levels_image_file_ref
      ...
      FOR EACH ROW
      EXECUTE FUNCTION object_store_public.populate_file_back_reference(''image'', ''constructive_status_public.app_levels'')';
    EXECUTE 'CREATE TRIGGER app_levels_image_file_delete
      BEFORE DELETE ON constructive_status_public.app_levels
      FOR EACH ROW
      EXECUTE FUNCTION object_store_public.mark_files_deleting_on_source_delete(''image'', ''constructive_status_public.app_levels'')';
  ...
  IF FOUND THEN
    EXECUTE 'CREATE TRIGGER sites_og_image_file_ref
      ...
      FOR EACH ROW
      EXECUTE FUNCTION object_store_public.populate_file_back_reference(''og_image'', ''services_public.sites'')';
    EXECUTE 'CREATE TRIGGER sites_og_image_file_delete
      BEFORE DELETE ON services_public.sites
      FOR EACH ROW
      EXECUTE FUNCTION object_store_public.mark_files_deleting_on_source_delete(''og_image'', ''services_public.sites'')';

    EXECUTE 'CREATE TRIGGER sites_apple_touch_icon_file_ref
      AFTER UPDATE OF apple_touch_icon ON services_public.sites
      FOR EACH ROW
      EXECUTE FUNCTION object_store_public.populate_file_back_reference(''apple_touch_icon'', ''services_public.sites'')';
    EXECUTE 'CREATE TRIGGER sites_apple_touch_icon_file_delete
      BEFORE DELETE ON services_public.sites
      FOR EACH ROW
      EXECUTE FUNCTION object_store_public.mark_files_deleting_on_source_delete(''apple_touch_icon'', ''services_public.sites'')';

    EXECUTE 'CREATE TRIGGER sites_logo_file_ref
      AFTER UPDATE OF logo ON services_public.sites
      FOR EACH ROW
      EXECUTE FUNCTION object_store_public.populate_file_back_reference(''logo'', ''services_public.sites'')';
    EXECUTE 'CREATE TRIGGER sites_logo_file_delete
      BEFORE DELETE ON services_public.sites
      FOR EACH ROW
      EXECUTE FUNCTION object_store_public.mark_files_deleting_on_source_delete(''logo'', ''services_public.sites'')';

    EXECUTE 'CREATE TRIGGER sites_favicon_file_ref
      AFTER UPDATE OF favicon ON services_public.sites
      FOR EACH ROW
      EXECUTE FUNCTION object_store_public.populate_file_back_reference(''favicon'', ''services_public.sites'')';
    EXECUTE 'CREATE TRIGGER sites_favicon_file_delete
      BEFORE DELETE ON services_public.sites
      FOR EACH ROW
      EXECUTE FUNCTION object_store_public.mark_files_deleting_on_source_delete(''favicon'', ''services_public.sites'')';
  ...
  IF FOUND THEN
    EXECUTE 'CREATE TRIGGER apps_app_image_file_ref
      ...
      FOR EACH ROW
      EXECUTE FUNCTION object_store_public.populate_file_back_reference(''app_image'', ''services_public.apps'')';
    EXECUTE 'CREATE TRIGGER apps_app_image_file_delete
      BEFORE DELETE ON services_public.apps
      FOR EACH ROW
      EXECUTE FUNCTION object_store_public.mark_files_deleting_on_source_delete(''app_image'', ''services_public.apps'')';
  ...
  IF FOUND THEN
    EXECUTE 'CREATE TRIGGER site_metadata_og_image_file_ref
      ...
      FOR EACH ROW
      EXECUTE FUNCTION object_store_public.populate_file_back_reference(''og_image'', ''services_public.site_metadata'')';
    EXECUTE 'CREATE TRIGGER site_metadata_og_image_file_delete
      BEFORE DELETE ON services_public.site_metadata
      FOR EACH ROW
      EXECUTE FUNCTION object_store_public.mark_files_deleting_on_source_delete(''og_image'', ''services_public.site_metadata'')';
  ...
  IF FOUND THEN
    EXECUTE 'CREATE TRIGGER migrate_files_upload_file_ref
      ...
      FOR EACH ROW
      EXECUTE FUNCTION object_store_public.populate_file_back_reference(''upload'', ''db_migrate.migrate_files'')';
    EXECUTE 'CREATE TRIGGER migrate_files_upload_file_delete
      BEFORE DELETE ON db_migrate.migrate_files
      FOR EACH ROW
      EXECUTE FUNCTION object_store_public.mark_files_deleting_on_source_delete(''upload'', ''db_migrate.migrate_files'')';
