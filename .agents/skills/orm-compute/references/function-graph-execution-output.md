...
db.functionGraphExecutionOutput.create({ data: { databaseId: '<UUID>', hash: '<Base64EncodedBinary>', data: '<JSON>' }, select: { id: true } }).execute()
db.functionGraphExecutionOutput.update({ where: { id: '<UUID>' }, data: { databaseId: '<UUID>' }, select: { id: true } }).execute()
...
const item = await db.functionGraphExecutionOutput.create({
  data: { databaseId: '<UUID>', hash: '<Base64EncodedBinary>', data: '<JSON>' },
  select: { id: true }
}).execute();
```
