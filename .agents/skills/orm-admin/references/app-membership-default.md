...
db.appMembershipDefault.create({ data: { createdBy: '<UUID>', createdByPrincipal: '<UUID>', isApproved: '<Boolean>', isVerified: '<Boolean>', updatedBy: '<UUID>', updatedByPrincipal: '<UUID>' }, select: { id: true } }).execute()
...
const item = await db.appMembershipDefault.create({
  data: { createdBy: '<UUID>', createdByPrincipal: '<UUID>', isApproved: '<Boolean>', isVerified: '<Boolean>', updatedBy: '<UUID>', updatedByPrincipal: '<UUID>' },
  select: { id: true }
}).execute();
```
