...
db.membershipType.create({ data: { name: '<String>', description: '<String>', prefix: '<String>', hasUsersTableEntry: '<Boolean>' }, select: { id: true } }).execute()
...
const item = await db.membershipType.create({
  data: { name: '<String>', description: '<String>', prefix: '<String>', hasUsersTableEntry: '<Boolean>' },
  select: { id: true }
}).execute();
```
