...
db.orgMembershipDefault.create({ data: { createdBy: '<UUID>', createdByPrincipal: '<UUID>', entityId: '<UUID>', isApproved: '<Boolean>', updatedBy: '<UUID>', updatedByPrincipal: '<UUID>' }, select: { id: true } }).execute()
...
const item = await db.orgMembershipDefault.create({
  data: { createdBy: '<UUID>', createdByPrincipal: '<UUID>', entityId: '<UUID>', isApproved: '<Boolean>', updatedBy: '<UUID>', updatedByPrincipal: '<UUID>' },
  select: { id: true }
}).execute();
```
