...
db.orgMembershipDefault.create({ data: { createdBy: '<UUID>', entityId: '<UUID>', isApproved: '<Boolean>', updatedBy: '<UUID>' }, select: { id: true } }).execute()
...
const item = await db.orgMembershipDefault.create({
  data: { createdBy: '<UUID>', entityId: '<UUID>', isApproved: '<Boolean>', updatedBy: '<UUID>' },
  select: { id: true }
}).execute();
```
