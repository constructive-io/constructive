...
# CRUD for any table (e.g. platform-config)
csdk platform-config list
csdk platform-config get --id <value>
csdk platform-config create --<field> <value>

# Non-interactive mode (skip all prompts, use flags only)
csdk --no-tty platform-config list
```
...
```bash
csdk --no-tty platform-config create --<field> <value>
```
