...
**L19/L20 are the function half of the same idea, and the larger one** (`checks/definer-function.ts`). A `SECURITY DEFINER` function's body runs as its owner, so every relation it touches is touched with the owner's privileges — `EXECUTE` is the only grant the caller needs, and no ACL on the base relation names it. `extractFunctionAccess` gives the privilege each reference exercises, not a read/write bit, and the walk follows calls: an inner definer re-owns the execution, an invoker callee keeps whichever owner is already in force, and a view read from a body continues through the L8 view walk. Two things are deliberately *not* reach: an invoker function (its body is the caller's own privileges) and a trigger function (Postgres refuses a direct call however wide its `EXECUTE` ACL, so `returnsTrigger` skips it).

L20 lifts the suppression L9 has carried since it shipped. A write against a view with `INSTEAD OF` triggers never reaches a base relation — it *becomes* the trigger function's body — and that body is permission-checked against the **function's** effective user. So the view's owner and its `security_invoker` setting decide nothing here; only `prosecdef` on the trigger function does. Verified on PG 18 in both directions, and both corpus cases invert the view's setting to keep the attribution honest. `ViewSnapshot.insteadOf` carries each trigger's target function and events (decoded from `tgtype`); an unresolvable or unreadable trigger function still suppresses, because a write goes *somewhere* and guessing where is the one thing this must not do.
...
**Which** file is the delta's whole meaning, and `gh run list --branch main --status success --limit 1` is the wrong answer: that is one unvalidated API row, and when the newest run hasn't concluded it walks back to an arbitrarily old one — 13.6 days, in the incident this exists for — so a PR that changed no SQL was charged with ~150 merges of accumulated debt. `safegres baseline --provider github` (the action's default, `compare: auto`) walks the base branch's recent runs and takes the newest that concluded `success`, is younger than 48h, audited a commit in this branch's **merge-base** history, and still has a downloadable, parseable report; it logs every rejection and writes `compare*` to `$GITHUB_OUTPUT` for the audit step. Nothing qualifying is not a failure: pass `--compare-skipped <why>` so the missing delta reads as "not measured" rather than "nothing changed", and let the absolute grade gate and the perf ratchet decide the job. Provenance (`--compare-sha`/`--compare-run-id`/`--compare-run-url`/`--compare-age`, or `toSnapshot(report, { ref, provenance })`) is what makes the rendered Δ checkable — an unattributed delta is how this went unnoticed for weeks. Needs `permissions: actions: read`. The guard itself is pure and provider-agnostic (`selectBaseline`).
...
safegres baseline --provider github  # CI: pick + download the run the Δ is measured against
...
Inside Actions the job summary, annotations and PR comment are emitted automatically (`report.github` configures them). The step itself can be the first-party composite action, `uses: constructive-io/constructive/packages/safegres@safegres-action-v1` (`packages/safegres/action.yml`): it installs the CLI, runs that config with `--github`, and takes only the inputs that legitimately differ between jobs sharing one config (`database`/`pgpm`, `fail-on-grade`, `report-only`, `out`, `comment`, `upload-sarif`, `upload-reports`, `compare`), exporting `security-score`/`security-grade`/`perf-score` as step outputs even on a gate failure. Anything you would pass every run belongs in the config file, not in `with:`. Add `failOn.grade` only once the baseline is stable. A second job that needs the same surface but different gates extends this file rather than copying it — `{ "extends": "../../.safegresrc.json", "failOn": { "grade": "D" } }`.
