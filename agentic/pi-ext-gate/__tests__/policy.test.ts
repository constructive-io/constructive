import { gatePolicy } from '../src';

describe('gatePolicy', () => {
  it('asks by default so an undeclared tool is never implicitly trusted', () => {
    expect(gatePolicy().evaluate({ toolName: 'bash', input: {} })).toEqual({ decision: 'ask' });
  });
  ...
  it('reports the rule that decided, so an audit trail can name it', () => {
    ...
    const verdict = gatePolicy({ rules: [rule] }).evaluate({ toolName: 'write', input: {} });
    expect(verdict.rule).toBe(rule);
  });

  it('rejects a rule with no tool name', () => {
    expect(() => gatePolicy({ rules: [{ tool: '', decision: 'allow' }] })).toThrow(/needs a tool name/);
  });
});
