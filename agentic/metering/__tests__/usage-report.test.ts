...
describe('toUsageReport', () => {
  ...
  it('keeps the full harness usage — cache splits and cost — as raw_usage', () => {
  ...
  it('derives a total when the harness reports none', () => {
  ...
  it('falls back to a generic error type when the harness gives no message', () => {
  ...
  it('rounds and clamps host-measured latency, and honours a custom operation', () => {
    expect(toUsageReport(message, { latencyMs: 1234.6, operation: 'agent/code-task' })).toMatchObject({
      latency_ms: 1235,
      operation: 'agent/code-task'
    });
