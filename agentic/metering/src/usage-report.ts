/**
 * Turn an assistant message's usage into a `POST /v1/usage` body.
 *
 * A harness reports richer usage than the gateway's own proxy path sees — cache
 * reads, cache writes, and its own cost breakdown — so the whole `usage` object is
 * sent as `raw_usage` while the four scalar columns carry what billing
 * aggregates.
 */

/**
 * The slice of an assistant message this module reads. An adapter maps its
 * harness's message onto it; the field names are the ones every provider SDK
 * already uses.
 */
...
export interface ToUsageReportOptions {
  /** Free-form label for the row; defaults to `agent/chat`. */
  operation?: string;
...
export function toUsageReport(
  ...
  const report: UsageReport = {
    ...
    service: 'chat',
    operation: options.operation ?? 'agent/chat',
