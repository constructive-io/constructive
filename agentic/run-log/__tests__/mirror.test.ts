import { MemoryRunLogStore, projectParts, type RunLogAppendStore, START } from '../src';
...
describe('SessionMirror', () => {
  ...
  it('passes the pi session version through to the records', async () => {
    const store = new MemoryRunLogStore();
    const session = fakeSession();
    const mirror = new SessionMirror({ runId: 'run-1', store, transcriptVersion: 3 });
    mirror.bind(session);
    session.push(message('user', 'a'));
    await mirror.drain();

    const page = await store.read('run-1', START);
    expect(page.records.every((r) => r.transcriptVersion === 3)).toBe(true);
  });
});
