...
class MockNotificationClient extends EventEmitter {
  ...
  readonly query = jest.fn(
    async (text: string, _values?: readonly unknown[]): Promise<unknown> => {
  ...
  readonly release = jest.fn(
    async (_error?: Error | boolean): Promise<void> => {}
  );
...
describe('PgNotificationBrokerRegistry', () => {
  it('shares one dedicated listener and reference-counts exact topics', async () => {
    ...
    const [first, second] = await Promise.all([
      registry.acquireForTests('opaque-a', sourceFactory, [
        'tenant.a',
        'shared',
      ]),
      registry.acquireForTests('opaque-a', sourceFactory, [
  ...
  it('audits three generations on the one pinned listener before admission', async () => {
    ...
    expect(source.connect).toHaveBeenCalledTimes(1);
    expect(
      client.queries.filter((query) => query === PG_NOTIFICATION_ROLE_AUDIT_SQL)
    ).toHaveLength(3);
    expect(client.queries).toEqual([
      'BEGIN READ ONLY',
  ...
  it('revalidates on the pinned listener and fails every lease closed on drift', async () => {
    ...
    await expect(firstNext).rejects.toBeInstanceOf(
      PgNotificationBrokerFailedError
    );
    await expect(secondNext).rejects.toBeInstanceOf(
      PgNotificationBrokerFailedError
    );
  ...
  it('uses exact topic equality for prefix and quoted-identifier channels', async () => {
    ...
    const lease = await registry.acquireForTests('opaque-a', () => source, [
      'tenant',
      'tenant.longer',
      hostileButValid,
    ]);
    ...
    expect(client.queries).toContain('LISTEN "tenant""; UNLISTEN *;--"');
    ...
    await expect(exact.next()).resolves.toEqual({
      done: false,
      value: 'exact',
    });
    await expect(longer.next()).resolves.toEqual({
      done: false,
      value: 'longer',
    });
    await expect(hostile.next()).resolves.toEqual({
      done: false,
      value: 'quoted',
    });
  ...
  it('rejects channels PostgreSQL would truncate, including multi-byte Unicode', async () => {
    ...
    await expect(
      registry.acquireForTests('opaque-b', () => source, [
  ...
  it('does not normalize canonically equivalent Unicode topics', async () => {
    ...
    await expect(composedStream.next()).resolves.toMatchObject({
      value: 'composed-only',
    });
    await expect(decomposedStream.next()).resolves.toMatchObject({
      value: 'decomposed-only',
    });
    await lease.release();
  });
  ...
  it('fans out only to subscribers for the exact allowed topic', async () => {
    const registry = new PgNotificationBrokerRegistry(4);
    const { client, source } = createSource();
    const first = await registry.acquireForTests('opaque-a', () => source, [
      'a',
    ]);
    const second = await registry.acquireForTests('opaque-a', () => source, [
      'a',
      'b',
    ]);
  ...
  it('fails only the slow subscriber when its bounded queue overflows', async () => {
    ...
    await expect(slow.next()).rejects.toBeInstanceOf(
      PgNotificationQueueOverflowError
    );
    expect(registry.stats()).toMatchObject({
      subscribers: 1,
  ...
  it('fails every active subscriber and never silently reconnects', async () => {
    ...
    const { client, source } = createSource();
    const sourceFactory = jest.fn(() => source);
    const first = await registry.acquireForTests('opaque-a', sourceFactory, [
      'a',
    ]);
    const second = await registry.acquireForTests('opaque-a', sourceFactory, [
      'b',
    ]);
    const firstNext = first.subscribe('a').next();
    const secondNext = second.subscribe('b').next();
  ...
  it('makes double release idempotent and awaits UNLISTEN plus both releases', async () => {
    ...
    (source.release as jest.Mock).mockImplementation(
      async () => sourceReleased.promise
    );
    const lease = await registry.acquireForTests('opaque-a', () => source, [
      'a',
    ]);
  ...
  it('bounds a never-resolving UNLISTEN so teardown cannot hang', async () => {
    ...
    try {
      const registry = new PgNotificationBrokerRegistry(4, 25);
      const { client, source } = createSource();
      await registry.acquireForTests('opaque-unlisten-timeout', () => source, [
        'a',
      ]);
      client.query.mockImplementation((text: string) => {
        client.queries.push(text);
  ...
  it('drains an in-flight acquisition before registry close resolves', async () => {
    ...
    const sourceReleased = deferred<void>();
    (source.connect as jest.Mock).mockImplementation(
  ...
  it('aggregates client and pool-lease cleanup failures after teardown attempts both', async () => {
    const registry = new PgNotificationBrokerRegistry(4);
    const { client, source } = createSource();
    client.release.mockRejectedValue(new Error('client release failed'));
    (source.release as jest.Mock).mockRejectedValue(
      new Error('source release failed')
    );
    await registry.acquireForTests('opaque-cleanup', () => source, ['a']);
...
describe('getPgNotificationBrokerIdentity', () => {
  ...
  it('is versioned, opaque, stable, and includes the canonical SSL contract', () => {
    const first = getPgNotificationBrokerIdentity({
      ...
      pool: {
        connectionTimeoutMillis: DEFAULT_PG_NOTIFICATION_OPERATION_TIMEOUT_MS,
      },
    });
  ...
  it('uses one credential-free identity for the same physical database target', () => {
    ...
    expect(first).toMatch(
      /^pg-notification-database:v1:pg-target:v1:[a-f0-9]{64}$/
    );
    expect(first).not.toContain('listener');
    expect(first).not.toContain('secret');
  });
