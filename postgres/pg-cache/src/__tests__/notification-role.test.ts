...
const poolWithRow = (row: Record<string, unknown> | undefined) => {
  const client = {
    query: jest.fn(async (query: string) =>
      query === PG_NOTIFICATION_ROLE_AUDIT_SQL
        ? { rows: row ? [row] : [] }
        : { rows: [] }
    ),
    release: jest.fn(),
...
describe('PostgreSQL notification-role audit', () => {
  ...
  it('audits an already-owned listener client without releasing it', async () => {
    ...
    ).resolves.toMatchObject({ ...contract, safe: true });
  ...
  it('rolls back a failed pinned-client audit without taking ownership of release', async () => {
    ...
    await expect(auditPgNotificationRoleClient(client, contract)).rejects.toBe(
      failure
    );
  ...
  it('reports both audit and rollback failures without releasing the pinned client', async () => {
    ...
    const rejected = auditPgNotificationRoleClient(client, contract);
    await expect(rejected).rejects.toBeInstanceOf(AggregateError);
    await expect(rejected).rejects.toMatchObject({
      cause: auditFailure,
      errors: [auditFailure, rollbackFailure],
    });
    expect(client.query).toHaveBeenNthCalledWith(4, 'ROLLBACK');
    expect(client.release).not.toHaveBeenCalled();
  });

  it.each<[keyof typeof safeRow, unknown, PgNotificationRoleViolationCode]>([
    ...
    'maps %s to its stable violation code',
    async (field, unsafeValue, code) => {
      ...
      await expect(
        ...
        audit: expect.objectContaining({
          violations: expect.arrayContaining([code]),
        }),
      });
    }
  );
  ...
  it('fails closed when the catalog audit returns no role row', async () => {
    ...
    expect(audit).toMatchObject({
      safe: false,
      violations: ['AUDIT_NO_RESULT'],
    });
    await expect(
      assertPgNotificationRole(poolWithRow(undefined).pool, contract)
...
describe('notification-role fleet contract', () => {
  ...
  it.each(malformedContracts)(
    'rejects malformed contract input',
    ({ contracts }) => {
      expect(() => normalizePgNotificationRoleContracts(contracts)).toThrow(
        PgNotificationRoleContractError
      );
    }
  );
