...
export const PG_NOTIFICATION_ROLE_UNSAFE_ERROR_CODE =
  'PG_NOTIFICATION_ROLE_UNSAFE';
...
const assertIdentifier = (
  ...
  if (value.includes('\0') || containsUnpairedSurrogate(value)) {
    throw new PgNotificationRoleContractError(
      `${kind} is not a valid PostgreSQL name`
    );
  }
...
): Readonly<PgNotificationRoleContract> =>
  Object.freeze({
...
export class UnsafePgNotificationRoleError extends Error {
  ...
  constructor(readonly audit: PgNotificationRoleAudit) {
    super(
...
export const normalizePgNotificationRoleContracts = (
  ...
  if (!Array.isArray(contracts) || contracts.length === 0) {
    throw new PgNotificationRoleContractError(
      'at least one role/database pair is required'
    );
  }
...
const violationCodes = (
  ...
  if (row.session_role !== contract.role)
    violations.push('LOGIN_ROLE_MISMATCH');
  if (row.active_role !== row.session_role)
    violations.push('CURRENT_ROLE_MISMATCH');
  if (row.active_database !== contract.database)
  ...
  if (row.other_database_connect_count > 0)
    violations.push('CROSS_DATABASE_CONNECT');
    ...
    if (inTransaction) {
      try {
        ...
        throw new AggregateError(
          [error, rollbackError],
          'PostgreSQL notification role audit and rollback both failed',
          { cause: error }
        );
      }
    }
