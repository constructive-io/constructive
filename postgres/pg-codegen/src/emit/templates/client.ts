 ...
 */
import type { Expr, Filter, LockOptions, LockStrength, Operand, SqlValue } from '@constructive-io/query-builder';
...
/**
 * Row-level lock the read takes, so a read-then-write stays serialized against
 * a concurrent one: `'update'` is `FOR UPDATE`, and the object form states what
 * to do about a row someone else holds.
 *
 * ```ts
 * const run = await runs.$with(tx).findFirst({ where: { id }, lock: 'update' });
 * const job = await jobs.$with(tx).findFirst({
 *   where: { status: 'pending' },
 *   lock: { strength: 'update', skipLocked: true }
 * });
 * ```
 */
export type Lock = LockStrength | ({ strength: LockStrength } & LockOptions);
...
export class TableClient<App> {
  ...
  async findMany<S extends SelectShape<App> | undefined = undefined>(
    ...
    if (args.lock) {
      const lock = typeof args.lock === 'string' ? { strength: args.lock } : args.lock;
      query.lock(lock.strength, { skipLocked: lock.skipLocked, noWait: lock.noWait });
    }
