...
it('generates the schema db factory', () => {
  expect(files['codegen_test/db.ts']).toMatchSnapshot();
});

it('copies the client runtime from the template', () => {
  expect(files['client.ts']).toContain('export class TableClient');
  expect(files['client.ts']).toContain('Generated by pg-codegen. DO NOT EDIT.');
  expect(files['client.ts']).not.toContain('read at codegen time');
});
