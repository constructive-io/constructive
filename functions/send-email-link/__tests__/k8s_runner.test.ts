...
describe('Kubernetes Remote Test Runner', () => {
    ...
    it('Should run tests inside a K8s Pod', async () => {
        ...
        const jobName = 'send-email-link-test-runner';
        const image = 'constructive/send-email-link-test:v3';

        // Optional: Build Image locally (if desired)
        // console.log('Building Docker Image...');
        // execSync('docker build -t constructive/send-email-link-test:v1 -f base/Dockerfile.test .', { stdio: 'inherit', cwd: process.cwd() + '/../../' });
