...
function createMockStream() {
  ...
  (emitter as any).destroy = jest.fn((_err?: Error) => {
    (emitter as any).destroyed = true;
  });
...
describe('GraphQLObjectType_fields_field hook (mutation resolver wrapping)', () => {
  ...
  it('should resolve upload promises and call custom resolver', async () => {
    ...
    const fakeUpload = { filename: 'photo.jpg', createReadStream: jest.fn().mockReturnValue(mockStream) };
    const args = {
      input: {
  ...
  it('should recursively resolve nested upload promises', async () => {
    ...
    const mockStream = createMockStream();
    const fakeUpload = { filename: 'deep.png', createReadStream: jest.fn().mockReturnValue(mockStream) };
    const args = {
      input: {
        nested: {
          photoUpload: Promise.resolve(fakeUpload)
...
describe('maxFileSize option', () => {
  function getUploadHooks(defs: UploadFieldDefinition[], opts: { maxFileSize?: number } = {}) {
    const plugin = createUploadPlugin({ uploadFieldDefinitions: defs, ...opts });
    return plugin.schema!.hooks as any;
  }
  ...
  async function consumeReadable(stream: Readable): Promise<number> {
    let total = 0;
  ...
  it('should reject files exceeding maxFileSize with a controlled error', async () => {
    const def: UploadFieldDefinition = {
      tag: 'upload', resolve: jest.fn(async (upload: any) => {
        const stream = upload.createReadStream();
        await consumeReadable(stream);
        return 'resolved-url';
      }),
      type: 'image'
    };

    const hooks = getUploadHooks([def], { maxFileSize: 100 });
    const build = createMockBuild();
    ...
    const wrappedField = hooks.GraphQLObjectType_fields_field(field, build, context);

    const fakeUpload = {
      filename: 'large.jpg',
      createReadStream: jest.fn().mockReturnValue(Readable.from([Buffer.alloc(150)]))
    };
    const args = { input: { photoUpload: Promise.resolve(fakeUpload) } };

    await expect(wrappedField.resolve({}, args, {}, {})).rejects.toThrow(
      'File exceeds maximum size of 100 bytes'
    );
  ...
  it('should allow files within maxFileSize', async () => {
    ...
    const hooks = getUploadHooks([def], { maxFileSize: 1000 });
    const build = createMockBuild();
    const context = createUploadContext();
    ...
    const fakeUpload = {
      filename: 'small.jpg',
      createReadStream: jest.fn().mockReturnValue(Readable.from([Buffer.alloc(50)]))
    };
    const args = { input: { photoUpload: Promise.resolve(fakeUpload) } };

    await wrappedField.resolve({}, args, {}, {});
    expect(oldResolve).toHaveBeenCalledTimes(1);
    expect((args.input as any).photo).toBe('resolved-url');
  });

  it('should enforce maxFileSize on the stream consumed by resolver', async () => {
    const def: UploadFieldDefinition = {
      tag: 'upload', resolve: jest.fn(async (upload: any) => {
        const stream = upload.createReadStream();
        await consumeReadable(stream);
    ...
    const fakeUpload = {
      ...
      createReadStream: jest.fn().mockReturnValue(Readable.from([Buffer.alloc(120)]))
    };
    const args = { input: { photoUpload: Promise.resolve(fakeUpload) } };
    ...
    // If plugin pre-read the stream before resolver, this would be 2 calls.
    expect(fakeUpload.createReadStream).toHaveBeenCalledTimes(1);
  });

  it('should enforce maxFileSize across multiple createReadStream calls', async () => {
    const def: UploadFieldDefinition = {
      tag: 'upload', resolve: jest.fn(async (upload: any) => {
        const stream1 = upload.createReadStream();
        await consumeReadable(stream1);

        const stream2 = upload.createReadStream();
        await consumeReadable(stream2);
    ...
    const fakeUpload = {
      ...
      createReadStream: jest.fn()
        .mockReturnValueOnce(Readable.from([Buffer.alloc(10)]))
        .mockReturnValueOnce(Readable.from([Buffer.alloc(150)]))
    };
    const args = { input: { photoUpload: Promise.resolve(fakeUpload) } };
    ...
    expect(fakeUpload.createReadStream).toHaveBeenCalledTimes(2);
    expect(oldResolve).not.toHaveBeenCalled();
  });
});
