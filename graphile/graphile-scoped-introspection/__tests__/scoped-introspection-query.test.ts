...
} from '../src/scoped-introspection-query';
...
describe('CNC-owned scoped introspection SQL', () => {
  ...
  it('returns normalized scope with the query plan', () => {
    ...
    expect(plan.query.values).toEqual([['tenant_a'], ['pg_trgm']]);
  });
  ...
  it('validates required root schemas without rejecting real dependencies', () => {
    const plan = makeSchemaScopedIntrospectionPlan(['tenant_a']);
    const introspection = {
      namespaces: [{ nspname: 'tenant_a' }, { nspname: 'shared_dependency' }],
      types: [],
      classes: [],
      attributes: [],
      constraints: [],
    ...
    expect(() => validateSchemaScopedIntrospection(introspection, plan)).not.toThrow();
  });
});
