...
describe('schema-scoped introspection runtime integration', () => {
  ...
  it('fails closed when a retained entity references a missing type', async () => {
    ...
    const failure = await makeSchema({
      extends: [
        graphileBuildPreset,
        graphileBuildPgPreset,
        ScopedIntrospectionPreset,
      ...
      gather: {
        pgScopedIntrospection: {
          [serviceName]: { catalogTypes: 'dependency-closure' },
        },
      },
      pgServices: [service as never],
    ...
    expect((failure as Error).message).toMatch(
      /validation failed for PostgreSQL service '.+': Dependency-closure introspection retained pg_class 'broken_items \(200\)' field 'reltype' referencing missing pg_type OID '999'/
    );
    expect(
