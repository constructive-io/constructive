...
describe('scoped introspection raw-text lifecycle', () => {
  it('reuses clean raw text, refreshes dirty data, and fails closed on errors', async () => {
    let cache: {
      introspectionResultsPromise: Promise<unknown> | null;
      dirty: boolean;
    } | null = null;
    let triggerRegather: (() => void) | null = null;
    let triggerDirtyRegather: (() => void) | null = null;
    ...
    const announcementPlugin = {
      name: 'ScopedIntrospectionAnnouncementObserverPlugin',
      gather: {
        namespace: 'scopedIntrospectionAnnouncementObserver',
        hooks: {
    ...
    const observerPlugin = {
      ...
      gather: {
        ...
        watch(_info: never, callback: () => void) {
          ...
          triggerDirtyRegather = () => {
            cache!.introspectionResultsPromise = null;
            cache!.dirty = true;
            callback();
          };
    ...
    try {
      ...
      expect(cache!.introspectionResultsPromise).not.toBeNull();
      ...
      triggerDirtyRegather!();
      const crossSchema = await results.next();
      expect(crossSchema.error).toBeUndefined();
      expect(crossSchema.input).toMatchObject({ namespaceName: SCHEMA });
