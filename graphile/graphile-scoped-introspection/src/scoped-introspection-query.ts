/**
 * Scoped catalog SQL adapted from Graphile Crystal's
 * utils/pg-introspection/src/scopedIntrospection.ts at commit
 * 441cef73b0a12ed24343b7f38241af28b6454280, which is MIT licensed
 * (Copyright © 2023 Benjie Gillam). This is a static, CNC-owned query
 * generator: it does not inspect or rewrite the upstream query at runtime.
 */
...
`;
/**
 * Builds a parameterized introspection plan scoped to the requested schemas
 ...
 *
 * Prefer `makeSchemaScopedIntrospectionPlan()` when the results will be
 * parsed and validated by this package.
 */
...
function assertDependencyClosureTypes(introspection: Introspection): void {
  ...
  const requireType = (
    ...
    // public arrays after lookup hydration; the lookup remains available.
    const resolves =
      retainedTypeOids.has(normalizedOid) ||
      introspectionLookups?.typeById?.has(normalizedOid) === true;
    if (!resolves) {
      throw new Error(
        `Dependency-closure introspection retained ${objectKind} '${objectContext}' field '${field}' referencing missing pg_type OID '${normalizedOid}'`
      );
    }
  };
  const requireTypes = (
