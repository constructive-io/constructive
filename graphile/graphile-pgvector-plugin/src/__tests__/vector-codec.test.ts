import { VectorCodecPlugin, VectorCodecPreset } from '../vector-codec';
...
describe('VectorCodecPlugin', () => {
  ...
  it('has a description', () => {
    expect(VectorCodecPlugin.description).toBeTruthy();
  });
});
...
describe('VectorCodecPlugin gather hook', () => {
  ...
  it('sets attributes to undefined (scalar, not composite)', async () => {
    const codec = await buildCodec();
    expect(codec.attributes).toBeUndefined();
  });

  it('sets executor to undefined', async () => {
    const codec = await buildCodec();
    expect(codec.executor).toBeUndefined();
  });
});
...
describe('VectorCodecPlugin codec.fromPg', () => {
  ...
  it('preserves floating point precision', () => {
    const result = fromPg('[0.123456789,0.987654321]');
    expect(result[0]).toBeCloseTo(0.123456789, 9);
    expect(result[1]).toBeCloseTo(0.987654321, 9);
  });

  it('parses zero vector', () => {
    expect(fromPg('[0,0,0]')).toEqual([0, 0, 0]);
  });
});
...
describe('VectorCodecPlugin codec.toPg', () => {
  ...
  it('serialises empty-ish values correctly', () => {
    expect(toPg([0])).toBe('[0]');
  });
});
...
describe('VectorCodecPlugin Vector scalar', () => {
  ...
  describe('serialize', () => {
    ...
    it('throws on object input', () => {
      expect(() => scalar.serialize({ x: 1 })).toThrow();
    });
  });
  ...
  describe('parseValue', () => {
    ...
    it('throws on null', () => {
      expect(() => scalar.parseValue(null)).toThrow();
    });
  });
  ...
  describe('parseLiteral', () => {
    ...
    it('parses ListValue of IntValues', () => {
      const ast = {
        kind: 'ListValue',
        values: [
          { kind: 'IntValue', value: '1' },
      ...
      expect(scalar.parseLiteral(ast)).toEqual([1, 2, 3]);
    });
  });

  describe('codec-to-scalar wiring', () => {
    it('calls setGraphQLTypeForPgCodec for both input and output', () => {
      const mockSetType = jest.fn();
      const vectorCodec = { name: 'vector' };
      const otherCodec = { name: 'text' };

      const mockBuild = {
        registerScalarType: jest.fn(),
        setGraphQLTypeForPgCodec: mockSetType,
        input: { pgRegistry: { pgCodecs: { v: vectorCodec, t: otherCodec } } },
      };
      ...
      // Should be called for vector codec only (input + output = 2 calls)
      expect(mockSetType).toHaveBeenCalledTimes(2);
      expect(mockSetType).toHaveBeenCalledWith(vectorCodec, 'input', 'Vector');
      expect(mockSetType).toHaveBeenCalledWith(vectorCodec, 'output', 'Vector');
    });

    it('does not wire non-vector codecs', () => {
      const mockSetType = jest.fn();

      const mockBuild = {
        registerScalarType: jest.fn(),
        setGraphQLTypeForPgCodec: mockSetType,
        input: { pgRegistry: { pgCodecs: { t: { name: 'text' }, j: { name: 'jsonb' } } } },
      };
