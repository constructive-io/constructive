...
describe('resolveIntrospectionSettings', () => {
  it('disables JIT only for scoped introspection', () => {
    const scoped = resolveIntrospectionSettings(
      'scoped-required',
      { statement_timeout: '5000', jit: 'on', work_mem: '16MB' }
    );
    const stock = resolveIntrospectionSettings('stock', { statement_timeout: '5000' });
