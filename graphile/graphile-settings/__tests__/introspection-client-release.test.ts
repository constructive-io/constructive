...
const {
  ...
  makeWithPgClientViaPgClientAlreadyInTransaction(client: unknown): TestWithPgClient;
};
...
describe('published dependency capability gate', () => {
  ...
  it('allows reuse without patched downstream dependencies', () => {
    expect(() => assertIntrospectionClientReleaseCapabilities('reuse', {
  ...
  it('requires both exact destroy protocol capabilities', () => {
    expect(() => assertIntrospectionClientReleaseCapabilities(
      'destroy',
      supported
    )).not.toThrow();
    expect(() => assertIntrospectionClientReleaseCapabilities('destroy', {
      ...supported,
      dataplanPg: undefined
    })).toThrow(
      'GRAPHILE_INTROSPECTION_CLIENT_DESTROY_UNSUPPORTED:@dataplan/pg'
    );
    expect(() => assertIntrospectionClientReleaseCapabilities('destroy', {
      ...supported,
      graphileBuildPg: undefined
...
describe('per-use PostgreSQL client release mode', () => {
  it('destroys the exact successful checkout once', async () => {
    ...
    expect(withPgClient.supportedClientReleaseModes).toEqual(['reuse', 'destroy']);
    await expect(withPgClient(
      null,
      async (client) => client.rawClient,
  ...
  it('fails closed before invoking an adaptor that does not advertise destruction', async () => {
    ...
    await expect(withPgClientFromPgService(
      service as never,
      null,
      callback,
      { clientReleaseMode: 'destroy' }
...
describe('introspection client release forwarding', () => {
  ...
  ] as const)('uses %s for the introspection checkout', async (
    _label,
    clientReleaseMode,
    expectedReleaseCalls
  ) => {
    const marker = new Error('captured introspection query');
    let sawIntrospection = false;
    const rawClient = makeRawClient();
    rawClient.query.mockImplementation(async (query: string | { text: string }) => {
      if (
        typeof query === 'object'
