...
export type ConstructivePgServiceOptions = Parameters<typeof makePostGraphilePgService>[0] & {
...
export const makePgService = (options: ConstructivePgServiceOptions) => {
  ...
  const introspectionScopedCatalogTypes = options.introspectionScopedCatalogTypes;
  const introspectionCapabilityExtensions = normalizeIntrospectionCapabilityExtensions(
    options.introspectionCapabilityExtensions
  );
  const introspectionClientReleaseMode = options.introspectionClientReleaseMode ?? 'reuse';
  if (
  ...
  if (introspectionMode === 'stock' && introspectionScopedCatalogTypes !== undefined) {
    throw new Error(
  ...
  const introspectionAllowedDependencySchemas = normalizeIntrospectionDependencySchemas(
