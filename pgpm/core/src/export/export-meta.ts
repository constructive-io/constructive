...
export const exportMeta = async ({ opts, dbname, database_id }: ExportMetaParams): Promise<ExportMetaResult> => {
  ...
  await queryAndParse('secure_table_provision', `SELECT *, COALESCE(fields, '{}') AS fields, COALESCE(grant_privileges, '{}') AS grant_privileges FROM metaschema_modules_public.secure_table_provision WHERE database_id = $1`);
