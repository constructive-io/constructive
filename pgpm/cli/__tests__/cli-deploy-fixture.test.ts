...
describe('CLIDeployTestFixture', () => {
  ...
  it('should handle CLI command execution with database', async () => {
    const commands = `pgpm migrate status --database ${testDb.name}`;
  ...
  it('should initialize workspace via CLI', async () => {
    const commands = `pgpm init workspace test-workspace`;
  ...
  it('should initialize module via CLI', async () => {
    const commands = `pgpm init test-module`;
    const results = await fixture.runTerminalCommands(commands, {}, false);
  ...
  it('should emulate terminal commands with database operations', async () => {
    const terminalCommands = `
      cd packages/
      pgpm deploy --recursive --database ${testDb.name} --yes --package my-first
      pgpm revert --recursive --database ${testDb.name} --yes --package my-first
    `;
  ...
  it('should work with fixture directories like sqitch-w-tags', async () => {
    const commands = `
      cd packages/
      pgpm deploy --recursive --database test_db --createdb --yes --package my-first
    `;
