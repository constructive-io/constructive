...
## Navigation
- Prev: [M_c1da9b300c28.md](M_c1da9b300c28.md) | Up: [ROOT.md](../ROOT.md) | Index: [README.md](../README.md) | Diffs: [diffs/README.md](README.md) | Snapshots: [SNAPSHOTS.md](../SNAPSHOTS.md) | Next: [M_7eef2f305cc7.md](M_7eef2f305cc7.md)
...
- Token deltas: `Constructive`: 3 → 8; `constructive`: 0 → 5; `launchql`: 3 → 0; `cnc`: 0 → 2; `GraphQLServer`: 0 → 1; `lql`: 1 → 0
- Headings added: `Common Workflows (via CLI)`, ``Constructive CLI (`constructive` / `cnc`)``, `Entry Points`, `GraphQL Server`, `Monorepo Layout`, ``PGPM CLI (`pgpm`)``, `Quick Start`, `Tips`
- Headings removed: `1. Module Development Workflow`, `2. Testing Workflow`, `3. Migration Workflow`, `CLI Command Structure`, `For API Development`, `For Database Operations`, `For File Operations`, `For Testing`, `Module Structure`, `PgpmMigrate Class`
...
- `This guide helps AI agents quickly navigate the Constructive monorepo. Constructive provides tooling for building secure, role-aware GraphQL APIs backed by PostgreSQL.`
  - `and understand` → ``
  - `codebase without having to read everything` → `monorepo`
  - `is` → `provides`
  - `a comprehensive full-stack framework` → `tooling`
  - `databases` → ``
...
- ``- **`PgpmPackage`** – `pgpm/core/src/core/class/pgpm.ts```
  - ``**Location:** `packages`` → ``- **`PgpmPackage`** – `pgpm``
...
## 🎯 Quick Start for Agents											      |	## Quick Start
**Most Important Packages to Know First:**									      |	**Most important packages to know first:**
1. **`packages/core`** - Main orchestration and migration engine ([detailed guide](packages/core/AGENTS.md))	      |	1. **`pgpm/core`** – PGPM engine: migrations, packaging, dependency resolution ([guide](pgpm/core/AGENTS.md))
2. **`packages/cli`** - Command-line interface and user workflows ([detailed guide](packages/cli/AGENTS.md))	      |	2. **`pgpm/pgpm`** – PGPM CLI: database/workspace commands (init/add/deploy/verify/etc)
3. **`packages/pgsql-test`** - Testing infrastructure ([detailed guide](packages/pgsql-test/AGENTS.md))		      |	3. **`packages/cli`** – Constructive CLI (`constructive` / `cnc`), delegates PGPM commands and adds GraphQL workflo
4. **`packages/server`** - GraphQL API server									      |	4. **`postgres/pgsql-test`** – PostgreSQL test harness and seed adapters ([guide](postgres/pgsql-test/AGENTS.md))
5. **`packages/types`** - TypeScript type definitions								      |	5. **`graphql/server`** – Constructive GraphQL server (Express + PostGraphile)
														      >	6. **`graphql/codegen`** – GraphQL code generation (types/ops/sdk)
**Key Classes to Understand:**											      |	**Key classes and entry points:**
- **`PgpmPackage`** (`packages/core/src/core/class/pgpm.ts`) - Workspace and module management			      |	- **`PgpmPackage`** – `pgpm/core/src/core/class/pgpm.ts`
- **`PgpmMigrate`** (`packages/core/src/migrate/client.ts`) - Database migration operations			      |	- **`PgpmMigrate`** – `pgpm/core/src/migrate/client.ts`
														      >	- **`GraphQLServer`** – `graphql/server/src/server.ts`
## 📦 Package Categories											      |	## Monorepo Layout
### 🏗️ Core Framework											      |	- **`packages/*`** – Constructive CLI + misc packages (`client`, `orm`, `query-builder`, `url-domains`, `server-uti
| Package | Purpose | Key Files |										      |	- **`pgpm/*`** – PGPM engine + CLI + shared types/logger/env
|---------|---------|-----------|										      |	- **`graphql/*`** – GraphQL server, explorer, codegen, types/env, query/react utilities
| **`core`** | Main orchestration, migrations, dependency resolution | `src/core/class/pgpm.ts`, `src/migrate/client. |	- **`postgres/*`** – PostgreSQL tooling and tests (`pg-ast`, `pg-codegen`, `introspectron`, `pgsql-test`, etc.)
| **`cli`** | Command-line interface (`lql` command) | `src/commands.ts`, `src/commands/deploy.ts` |		      |	- **`streaming/*`** – S3 helpers and stream hashing utilities
| **`types`** | TypeScript type definitions | `src/pgpm.ts` |							      |	- **`extensions/*`** – PGPM extension modules (Postgres extensions packaged as PGPM modules)
| **`env`** | Environment and configuration management | - |							      |	- **`graphile/*`** – Graphile/PostGraphile plugins (kept under their own namespace)
...
### 🧠 Parsing & AST												      |	### Constructive CLI (`constructive` / `cnc`)
| Package | Purpose | Key Files |										      <	
|---------|---------|-----------|										      <	
| **`pg-ast`** | PostgreSQL AST tools and transformations | - |							      <	
| **`gql-ast`** | GraphQL AST utilities | - |									      <	
### 🔁 Streaming & File Handling										      |	- **Router:** `packages/cli/src/commands.ts`
...
### 🏗️ Query Building & Code Generation									      |	### GraphQL Server
| Package | Purpose | Key Files |										      <	
|---------|---------|-----------|										      <	
...
### 🛠️ Development Tools											      |	- **Entry:** `graphql/server/src/index.ts`
...
## 🔑 Key Classes and Entry Points										      |	## Common Workflows (via CLI)
...
- `deploy(opts, target?, recursive?)` - Deploy modules to database						      |	2. Use `packages/cli/AGENTS.md` to understand Constructive’s command routing.
