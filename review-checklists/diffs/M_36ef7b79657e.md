# Diff — `M` `packages/cli/AGENTS.md`

## Navigation
- Prev: [D_1d58a19ddb53.md](D_1d58a19ddb53.md) | Up: [packages/cli.md](../packages/cli.md) | Index: [README.md](../README.md) | Diffs: [diffs/README.md](README.md) | Snapshots: [SNAPSHOTS.md](../SNAPSHOTS.md) | Next: [M_aaab89ad9ede.md](M_aaab89ad9ede.md)
...
- Token deltas: `lql`: 24 → 0; `Constructive`: 0 → 8; `LaunchQL`: 6 → 0; `@constructive-io/`: 0 → 3; `@launchql/`: 2 → 0; `constructive`: 2 → 4; `launchql`: 2 → 0; `cnc`: 0 → 1
- Headings added: `Command Sources`, `Constructive CLI Package - Agent Guide`, `Constructive GraphQL Commands (local)`, `Debugging Tips`, `Entry Points`, `PGPM Commands (delegated)`, `Tests`
- Headings removed: `1. Deploy Command`, `1. New Project Setup`, `2. Init Command`, `2. Module Development`, `3. Production Deployment`, `3. Server Command`, `4. Migrate Command`, `4. Migration Management`, `5. Extension Command`, `Add dependencies`
...
## Key removals
- ``The `@launchql/cli` package provides the command-line interface for LaunchQL, exposing all framework functionality through the `lql` command. This guide helps agents understand the CLI structure and …``
...
## 🎯 Overview												      |	- **Binaries:** `constructive` (full) and `cnc` (shorthand)
...
## 🏗️ CLI Architecture											      |	### PGPM Commands (delegated)
...
```														      <	
## 🚀 Core Commands												      |	### Constructive GraphQL Commands (local)
...
**Usage:**													      |	- `packages/cli/src/commands/server.ts` – start the Constructive GraphQL server
```bash														      |	- `packages/cli/src/commands/explorer.ts` – start the Constructive GraphQL explorer
lql deploy [OPTIONS]												      |	- `packages/cli/src/commands/get-graphql-schema.ts` – emit schema SDL (DB build or endpoint introspection)
```														      |	- `packages/cli/src/commands/codegen.ts` – run GraphQL codegen (`@constructive-io/graphql-codegen`)
