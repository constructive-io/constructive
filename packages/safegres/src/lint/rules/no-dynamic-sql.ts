...
export const noDynamicSql: LintRule = {
  ...
  run(unit) {
    return unit.dynamicSql.map((site) => ({
      ...
      hint: 'Avoid dynamic SQL. If it is genuinely lookup-only or code-generation (never a write), waive it with a reason: `-- safegres-disable-next-line no-dynamic-sql -- lookup-only: <why>`.',
