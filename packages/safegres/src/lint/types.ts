 ...
 *
 * This module is deliberately free of any `pg` / catalog dependency: it takes
 * a function *definition* (the `CREATE FUNCTION …` text, as `pg_get_functiondef`
 * returns it, or as authored in a migration) and returns findings. That keeps
 * it mechanically liftable into a standalone `@pgsql/lint` package if a second
 * consumer ever appears; safegres is just the first one.
 */
...
export interface LintRuleMeta {
  ...
  id: string;
  /** safegres registry code this rule maps to, e.g. `C4`. */
  code: string;
  title: string;
  /**
   * Whether a suppression of this rule must carry a reason. When true, a bare
   * `safegres-disable*` directive does not suppress — the finding stands — so
   * a waiver is never silent. Only `no-dynamic-sql` requires it: it is the one
   * rule we expect to be waived (lookup-only / codegen), and the waiver's whole
   * value is the documented reason.
   */
