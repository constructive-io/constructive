 ...
 * authored in a migration survives `pg_get_functiondef` and is visible to the
 * live-database audit — the comment is the single source of truth.
 *
 * Grammar (a `--` comment containing):
 *
 *   safegres-disable-next-line [<rule>…] [-- <reason>]   next physical line
 *   safegres-disable-line      [<rule>…] [-- <reason>]   this physical line
 *   safegres-disable           [<rule>…] [-- <reason>]   until a matching enable
 *   safegres-enable            [<rule>…]                 closes a disable range
 *   safegres-disable-file      [<rule>…] [-- <reason>]   the whole definition
 *
