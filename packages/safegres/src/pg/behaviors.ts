...
export function emptyBehaviorSnapshot(): BehaviorSnapshot {
  return { tables: new Map(), columns: new Map(), constraints: new Map() };
}
...
export function parseBehaviorTag(comment: string | null | undefined): string | null {
  ...
  for (const line of comment.split('\n')) {
    ...
    const match = /^@behavior\s+(.+)$/.exec(trimmed);
...
export async function introspectBehaviors(
  ...
  for (const row of rows) {
    const behavior = parseBehaviorTag(row.description);
    if (!behavior) continue;
    const relation = `${row.schema_name}.${row.table_name}`;
    if (row.kind === 'table') snapshot.tables.set(relation, behavior);
    else if (row.member) {
      const map = row.kind === 'column' ? snapshot.columns : snapshot.constraints;
      map.set(`${relation}.${row.member}`, behavior);
    }
  }
  return snapshot;
}
