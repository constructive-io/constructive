...
export const SECURITY_GUC_KEYS = [
  ...
  'jwt.claims.root_session_id',
  'jwt.claims.parent_session_id',
  'jwt.claims.intent',
  'jwt.claims.entity_type',
...
export function buildPgSettings(input: PgSettingsInput): PgSettings {
  ...
  setClaim(settings, 'jwt.claims.root_session_id', token?.root_session_id);
  setClaim(settings, 'jwt.claims.parent_session_id', token?.parent_session_id);
  setClaim(settings, 'jwt.claims.intent', token?.intent);
  setClaim(settings, 'jwt.claims.entity_type', token?.entity_type);
  ...
  // Preserve database attribution for requests without an authenticated actor.
