/**
 * fleet canary — seed one CANARY-<dbname> row into each tenant's app-public
 * categories table (superuser pg client). Idempotent. Port of
 * `scripts/scale-validate/canary-seed.mjs`.
 *
 ...
 *
 *   perf-harness fleet canary --fleet fleet.json
 *   perf-harness fleet canary --fleet fleet.json --cleanup
 */
import { Argv, asBool, usageExit } from '../core/args';
import { pgConfigFromArgv } from '../core/config';
import { canaryName, loadFleet } from '../core/fleetfile';
import { withFreshClient } from '../core/pgc';

export const USAGE = `perf-harness fleet canary — insert/remove CANARY-<dbname> rows in each tenant's categories
...
export function valueForColumn(
  col: { column_name: string; data_type?: string; udt_name?: string },
  nameValue: string,
  slugValue: string
): any {
...
export async function run(argv: Argv): Promise<number> {
  ...
  try {
    const table = typeof argv.table === 'string' ? argv.table : 'categories';
    const cleanup = asBool(argv.cleanup);
    const { tenants } = loadFleet(argv.fleet);
    const cfg = pgConfigFromArgv(argv);

    const report: any = { table, mode: cleanup ? 'cleanup' : 'seed', results: [] };
    await withFreshClient(cfg, async (client) => {
      for (const t of tenants) {
        const schema = t.appPublicSchema;
        const entry: any = { dbname: t.dbname, schema, status: 'ok' };
        if (!schema) {
        ...
        // canary token == tenant.dbname (see core/fleetfile); slug is a
        // filesystem-safe rendering of it.
        const slug = `canary-${t.dbname}`.toLowerCase().replace(/[^a-z0-9]+/g, '-');
        ...
        const already = await client.query(
          `SELECT count(*)::int AS n FROM ${qSchema}.${qTable} WHERE name = $1`,
          [name]
        );
    ...
    const seeded = report.results.filter((r: any) => r.inserted > 0).length;
    const existed = report.results.filter((r: any) => r.status === 'exists').length;
    const removed = report.results.reduce((a: number, r: any) => a + (r.removed || 0), 0);
    const skipped = report.results.filter((r: any) => r.status === 'skipped').length;
