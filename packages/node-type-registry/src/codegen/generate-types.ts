...
function buildRelationTypes(
  ...
  const relationMembers: t.TSType[] = relationNodes.map((nt) => {
    const baseMembers: t.TSTypeElement[] = [
      ...
      addJSDoc(
        optionalProp('source_table', t.tsStringKeyword()),
        'Name of the source table. Required unless the end is given as source_module.'
      ),
      addJSDoc(
        optionalProp('target_table', t.tsStringKeyword()),
        'Name of the target table. Required unless the end is given as target_module.'
      ),
      ...
      addJSDoc(
        optionalProp(
          'source_module',
          t.tsTypeReference(t.identifier('BlueprintModuleTableRef'))
        ),
        'Point the source end at a table an installed module generated, e.g. { type: "agent", scope: "org", table: "message" }. Mutually exclusive with source_table/source_schema.'
      ),
      addJSDoc(
        optionalProp(
          'target_module',
        ...
        'Point the target end at a table an installed module generated, e.g. { type: "storage", scope: "app", table: "files" }. Mutually exclusive with target_table/target_schema.'
      )
    ];
...
/**
 * Build the BlueprintModuleTableRef interface.
 *
 * A table entry may point at a table an installed module generated instead of
 * naming one: the reference resolves through the module row's recorded table
 * ids, so it survives whatever prefix, name or schema the module chose.
 */
function buildBlueprintModuleTableRef(): t.ExportNamedDeclaration {
  return addJSDoc(
    exportInterface('BlueprintModuleTableRef', [
      addJSDoc(
        requiredProp('type', t.tsStringKeyword()),
        'Module type owning the table. Currently "agent".'
      ),
      addJSDoc(
        requiredProp('table', t.tsStringKeyword()),
        'Logical table key within the module, not a table name. Agent: "thread", "message", "task", "prompt", "plan", "agent", "persona", "resource".'
      ),
      addJSDoc(
        optionalProp('scope', t.tsStringKeyword()),
...
/**
 * Build the BlueprintModuleInstallRef interface.
 *
 * A reference to one module *install* rather than one of its tables, for an
 * entry whose provisions map already names the tables.
 */
function buildBlueprintModuleInstallRef(): t.ExportNamedDeclaration {
  return addJSDoc(
    exportInterface('BlueprintModuleInstallRef', [
      addJSDoc(
        requiredProp('type', t.tsStringKeyword()),
...
function buildBlueprintAgentConfig(): t.ExportNamedDeclaration {
  return addJSDoc(
    exportInterface('BlueprintAgentConfig', [
      ...
      addJSDoc(
        optionalProp('has_runs', t.tsBooleanKeyword()),
    ...
    exportInterface('BlueprintTable', [
      ...
      addJSDoc(
        optionalProp(
          ...
          t.tsUnionType([
            t.tsTypeReference(t.identifier('BlueprintModuleTableRef')),
            t.tsTypeReference(t.identifier('BlueprintModuleInstallRef'))
          ])
        ),
...
function buildProgram(meta?: MetaTableInfo[]): string {
  ...
  statements.push(buildBlueprintModuleTableRef());
  statements.push(buildBlueprintModuleInstallRef());
