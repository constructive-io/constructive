...
/**
 * `auth:hardened` — email/password auth with rate limiting, SSO, passkeys, SMS,
 ...
 * multi-tenant B2B, step up to `b2b:storage`.
 */
export const PresetAuthHardened: ModulePreset = {
  ...
  description:
    'The full email/password auth stack, plus every optional auth module that fits inside the single-tenant ' +
  ...
  not_for: [
    'Multi-tenant B2B apps — use `b2b:storage`, which layers orgs + invites + permissions on top'
  ],
  modules: [
    'users_module',
