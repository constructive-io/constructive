...
export interface CacheSizeConfig {
  graphileCacheMax?: number;
  multiTenancyCacheMax?: number;
  pgCacheMax?: number;
}
...
export interface PerfRunConfig {
  ...
  effectiveCacheEnv: Record<string, string>;
  failFast: boolean;
  captureMemory: boolean;
  routeProbeSampleSize: number;
  errorSampleLimit: number;
  testTimeoutMs: number;
  constructiveLocalPath: string;
...
> & {
  ...
  cacheSizes?: Partial<CacheSizeConfig>;
  constructiveLocalPath?: string;
};
...
export interface RuntimeOperationProfile extends OperationProfile {
  ...
  requiresExistingRow?: boolean;
  producesRowId?: boolean;
}
...
export interface MemoryCaseArtifacts {
  beforePath?: string;
  afterPath?: string;
  beforeOk: boolean;
  afterOk: boolean;
