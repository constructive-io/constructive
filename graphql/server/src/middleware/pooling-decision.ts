...
export const computePoolDecision = async (
  ...
  try {
    ...
    // Structural opt-out: bm25-indexed shapes are not poolable. The bm25
    // adapter embeds the build-time schema-qualified index name as a bind
    // VALUE, which the rewrite seam deliberately never touches — a shared
    // instance would aim every tenant's bm25 scoring at the canonical
    // tenant's index (see hasBm25Indexes). Memoized like any structural
    // decision; a schema change flush re-probes.
    if (await hasBm25Indexes(pool, api.schema)) {
      log.info(
        `svc=${svcKey} not poolable — schemas carry bm25 indexes; using per-tenant instance`
      );
      return { key: svcKey, pooling: false };
    }
