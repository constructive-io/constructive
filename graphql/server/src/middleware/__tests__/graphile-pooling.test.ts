...
describe('computePoolDecision', () => {
  ...
  it('pools a clean single-tenant shape and returns a stable bp: key', async () => {
    ...
    expect(query).toHaveBeenCalledTimes(2); // shape scan + bm25 probe, nothing else
  });
...
describe('resolvePoolDecision caching', () => {
  ...
  it('memoizes the decision per svc_key (one probe pair) until cleared', async () => {
    ...
    expect(query).toHaveBeenCalledTimes(2); // one probe pair — second call served from cache

    // clearPoolDecisions() forces the next request to re-probe (a fresh pair).
    clearPoolDecisions();
...
describe('transient probe failures are not memoized (W3 fix)', () => {
  ...
  it('re-probes on the next request after a thrown catalog probe', async () => {
    ...
    expect(query).toHaveBeenCalledTimes(3); // failed probe, then a full pair
  });
});

describe('bm25 structural opt-out', () => {
  afterEach(() => {
    clearPoolDecisions();
    jest.clearAllMocks();
  });

  it('does not pool a shape that carries bm25 indexes (memoized structural opt-out)', async () => {
    const { pool, query } = makePool([
      { rows: [{ nspname: 'shop-5e6b13b2-app-public', relname: 'documents' }] },
      { rows: [{ exists: true }] }
    ]);
    const api = makeApi({ schema: ['shop-5e6b13b2-app-public'] });

    const decision = await resolvePoolDecision('svc-bm25', api, pool);
    expect(decision.pooling).toBe(false);
    expect(decision.key).toBe('svc-bm25');
    expect(decision.transient).toBeUndefined(); // structural: memoized until flush

    const second = await resolvePoolDecision('svc-bm25', api, pool);
    expect(second).toBe(decision); // cached — no re-probe
    expect(query).toHaveBeenCalledTimes(2);
  });

  it('pools when the bm25 probe finds no bm25 indexes', async () => {
    const { pool } = makePool([
      { rows: [{ nspname: 'shop-5e6b13b2-app-public', relname: 'orders' }] },
      { rows: [{ exists: false }] }
    ]);
    ...
    const decision = await computePoolDecision('svc-nobm25', api, pool);
    expect(decision.pooling).toBe(true);
    expect(decision.key).toMatch(/^bp:[0-9a-f]{64}$/);
  });

  it('a thrown bm25 probe falls back transiently (same contract as the shape probe)', async () => {
    const query = jest.fn();
    query.mockResolvedValueOnce({
      rows: [{ nspname: 'shop-5e6b13b2-app-public', relname: 'orders' }]
    });
    ...
    const decision = await computePoolDecision('svc-bm25-err', api, { query } as unknown as Pool);
    expect(decision).toEqual({ key: 'svc-bm25-err', pooling: false, transient: true });
  });
});
