...
const log = new Logger('oauth:google');
...
export const mountGoogle: ProviderBuilder['mount'] = (opts, cfg, deps) => {
  const router = express.Router();
  if (!cfg?.clientId || !cfg.clientSecret) {
    log.warn('Google OAuth not configured; skipping /auth/google routes');
    return router;
  }
  ...
  const defaultCallbackUrl = `http://${defaultHost}:${serverPort}/auth/google/callback`;
  ...
  const verifyCallback = async (
    ...
    profile: any,
        ...
        try {
          ...
          if (!emailInfo) {
            log.error('Google profile missing email', { profile });
            return done(new Error('google profile missing email'));
          }

          log.debug('Email extracted from Google profile', { email: emailInfo.email, verified: emailInfo.verified });
  ...
  router.get(
    ...
    (req: express.Request, res: express.Response, next: express.NextFunction) => {
      ...
      const dynamicCallbackUrl = `${protocol}://${hostWithPort}/auth/google/callback`;
      ...
      log.debug('Initiating Google OAuth', {
        domain,
        subdomain,
        hostname: req.hostname,
        host: req.get('host'),
      ...
      // Manually build the Google authorization URL with correct callback URL
      ...
      log.debug('Redirecting to Google OAuth', {
        authUrl: authUrl.toString(),
        callbackUrl: dynamicCallbackUrl,
      });
      
      // Redirect to Google with correct callback URL
  ...
  router.get('/callback', (req, res, next) => {
    ...
    passport.authenticate(strategy, { session: false }, (err: any, data: UpsertResult | undefined) => {
      if (err) {
        ...
        log.error(`Google OAuth callback error: ${errorName}: ${errorMessage}`, {
          name: errorName,
      ...
      if (!data?.accessToken) {
        log.error('Google OAuth callback error: No access token', {
          data,
          query: req.query,
        });
      ...
      if (redirectUrl) {
        try {
          ...
          url.searchParams.set('data', JSON.stringify({ ...data, provider: 'google' }));
...
export const googleProvider: ProviderBuilder = {
  name: 'google',
  scope: ['profile', 'email'],
  mount: mountGoogle,
};
