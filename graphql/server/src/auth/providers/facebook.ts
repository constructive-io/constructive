...
const log = new Logger('oauth:facebook');

const pickEmail: EmailPicker = (profile: any): { email: string; verified: boolean } | null => {
  // Facebook profile emails are in profile.emails array
...
export const mountFacebook: ProviderBuilder['mount'] = (opts, cfg, deps) => {
  const router = express.Router();
  if (!cfg?.clientId || !cfg.clientSecret) {
    log.warn('Facebook OAuth not configured; skipping /auth/facebook routes');
    return router;
  }
  ...
  const defaultCallbackUrl = `http://${defaultHost}:${serverPort}/auth/facebook/callback`;
  ...
  const verifyCallback = async (
    ...
    try {
      ...
      if (!emailInfo) {
        log.error('Facebook profile missing email', { profile });
        return done(new Error('facebook profile missing email'));
      }

      log.debug('Email extracted from Facebook profile', { email: emailInfo.email, verified: emailInfo.verified });
  ...
  router.get(
    ...
    (req: express.Request, res: express.Response, next: express.NextFunction) => {
      ...
      const dynamicCallbackUrl = `${protocol}://${hostWithPort}/auth/facebook/callback`;
      ...
      log.debug('Initiating Facebook OAuth', {
        domain,
        subdomain,
        hostname: req.hostname,
        host: req.get('host'),
      ...
      // Manually build the Facebook authorization URL with correct callback URL
      ...
      log.debug('Redirecting to Facebook OAuth', {
        authUrl: authUrl.toString(),
        callbackUrl: dynamicCallbackUrl,
      });
      
      // Redirect to Facebook with correct callback URL
  ...
  router.get('/callback', (req, res, next) => {
    ...
    if (callbackUrl === defaultCallbackUrl) {
      ...
      callbackUrl = `${protocol}://${host}/auth/facebook/callback`;
    }
    ...
    passport.authenticate(strategy, { session: false }, (err: any, data: UpsertResult | undefined) => {
      if (err) {
        ...
        log.error(`Facebook OAuth callback error: ${errorName}: ${errorMessage}`, {
          name: errorName,
      ...
      if (!data?.accessToken) {
        log.error('Facebook OAuth callback error: No access token', {
          data,
          query: req.query,
        });
      ...
      log.info('Facebook OAuth callback successful', {
        userId: data.userId,
        email: data.email,
      });
      ...
      if (redirectUrl) {
        try {
          ...
          url.searchParams.set('data', JSON.stringify({ ...data, provider: 'facebook' }));
...
export const facebookProvider: ProviderBuilder = {
  name: 'facebook',
  scope: ['email'],
  mount: mountFacebook,
};
