...
describe('removeStaleTargetDirs', () => {
  ...
  /** Create a directory with a generated marker file inside it. */
  function mkGeneratedDir(name: string) {
    const dir = path.join(tempDir, name);
    fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(path.join(dir, 'index.ts'), GENERATED_HEADER);
  }

  /** Create a directory with only hand-written (non-generated) files. */
  ...
  it('detects generated marker in nested subdirectories', () => {
    const nested = path.join(tempDir, 'old-target', 'orm');
    fs.mkdirSync(nested, { recursive: true });
    fs.writeFileSync(path.join(nested, 'client.ts'), GENERATED_HEADER);
  ...
  it('skips empty directories (no generated files)', () => {
